...
PAGES=$(pdfinfo "$FINAL" | grep "^Pages:" | awk '{print $2}')
if [ "$PAGES" -gt 10 ]; then
  echo "  ERROR: Page count $PAGES exceeds maximum of 10."
  exit 1
else
  echo "  Page count: $PAGES — within limit."
fi
