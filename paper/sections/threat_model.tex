...
The \textsc{Replace-Fixed-Budget} mechanism samples replacement scores with replacement from the tail reservoir; at high fractions ($f{=}0.4$), this may produce duplicate scores in the calibration buffer, which is detectable by a uniqueness check---this limitation is disclosed in \S\ref{sec:discussion}.

\textbf{Attack mechanics.}
We evaluate two attack objectives:
\begin{description}
  \item[Raise.] \textsc{Threshold-Raise} (primary): replace $m{=}\lfloor f \cdot n_v \rfloor$ randomly selected positions in $\mathcal{C}_v$ with scores drawn with replacement from the top-10\% tail of $\mathcal{C}_v$ (\textsc{High-Score-Benign} source).
...
The score-level characterization is a controlled experiment, not a deployed attack.
