...
\begin{description}
  \item[Raise.] \textsc{Threshold-Raise} (primary): for $f{>}0$, replace $m=\max(1,\mathrm{round}(fn_v))$ random positions in $\mathcal{C}_v$ with scores drawn with replacement from the top-10\% tail of $\mathcal{C}_v$ (\textsc{High-Score-Benign} source).
