...
from datp.experiments.models import PipelineRequest, SweepStep
...
def _score_only_recovery(
    request: PipelineRequest, ckpt_dir: Path, ckpt_file: Path
) -> None:
    import torch
    ...
    model = load_model_from_checkpoint(
        request.cfg, ckpt_dir=ckpt_dir, require_cuda=request.cfg.machine.require_cuda
    )
...
def _handle_non_protocol_checkpoint(
    ...
    try:
        ...
        logger.info(
            "checkpoint exists, skipping training", stage=key.stage, seed=key.seed
        )
        return
