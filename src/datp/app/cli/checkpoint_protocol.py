...
def summary(
    ...
    metrics = _load_metrics(artifact_root=artifact_root, seeds=tuple(seeds), rounds=tuple(rounds))
    selection = select_global_primary_checkpoint(
        metrics=metrics,
        n_bootstrap=BASE_CONFIG.statistics.n_bootstrap,
        bootstrap_seed=BASE_CONFIG.statistics.bootstrap_seed,
    )
    _stdout.print(json.dumps({"selected_round": selection.selected_round}, sort_keys=True))
...
def _write_smoke_fixture(artifact_root: Path) -> tuple[SweepMetrics, ...]:
    ...
    for checkpoint_round in (25, 50):
        for seed in (0, 1, 2):
            ...
            ckpt_path.write_text(f"fake checkpoint {seed} {checkpoint_round}\n", encoding="utf-8")
            _write_manifest(layout, cell, checkpoint_round, ckpt_path)
    for metric in metrics:
        cell = TrainingCellId(regime=metric.regime, seed=metric.seed, alpha=metric.alpha)
        ...
        metrics_path = layout.baseline_run_for_round(
            run, checkpoint_round
        ).metrics_path
