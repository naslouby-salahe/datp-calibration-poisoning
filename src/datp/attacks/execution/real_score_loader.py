"""Load real datp-cp generated N-BaIoT clean scores into a score collection.

Read-only: wraps the existing datp-cp scoring loaders (``cal_loading``,
...
def load_real_score_collection(
    *,
    ...
    """Build a ``ScoreCollection`` from real per-client datp-cp score artifacts.
