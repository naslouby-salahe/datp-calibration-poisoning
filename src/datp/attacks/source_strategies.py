"""Source strategy dispatch for CP2 calibration poisoning.
...
_MVP_SOURCES: frozenset[PoisoningSourceStrategy] = frozenset(
    {
...
def is_diagnostic_source(source: PoisoningSourceStrategy) -> bool:
    """Return True if source is diagnostic-only (not for MVP or full matrix)."""
...
def select_reservoir(
    *,
    ...
    For MVP sources, delegates directly to build_reservoir.
    """
    if is_diagnostic_source(source) and not allow_diagnostic:
        raise DiagnosticSourceError(
            ...
            "to use it explicitly.  It must not enter the MVP/full matrix."
        )
