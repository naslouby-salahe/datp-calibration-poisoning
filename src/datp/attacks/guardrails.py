"""CP2 runtime scientific guardrails.

Each guardrail function asserts one protocol invariant and raises
``Cp2GuardrailError`` (a ``ValueError``) on violation. Call these at
...
class Cp2GuardrailError(ValueError):
    """Raised when a CP2 protocol guardrail is violated at runtime."""
...
def assert_no_inplace_mutation(
    ...
    *after* the operation. If they differ, the operation mutated in place.
    ...
    Raises:
        Cp2GuardrailError: If the arrays are not equal (in-place mutation detected).
    """
    if not np.array_equal(original, after):
        raise Cp2GuardrailError(
            f"CP2 guardrail: clean {label} was mutated in place. "
...
def assert_reservoir_not_test_or_training(reservoir_label: str) -> None:
    ...
    CP2 reservoirs must be victim-local benign *calibration* scores only.
    ...
    Raises:
        Cp2GuardrailError: If any forbidden term appears in the label.
    """
    lower = reservoir_label.lower()
    for term in _FORBIDDEN_RESERVOIR_TERMS:
        if term in lower:
            raise Cp2GuardrailError(
                f"CP2 guardrail: reservoir label {reservoir_label!r} "
...
def assert_policy_not_b3(policy: ThresholdPolicy) -> None:
    """Raise if ``policy`` is B3, which is excluded from CP2.

    B3 (conformal threshold) is not part of the CP2 default policy enum.
    This is a defence-in-depth check; the enum itself cannot represent B3.

    Raises:
        Cp2GuardrailError: If the policy string contains "b3".
    """
    if "b3" in str(policy).lower():
        raise Cp2GuardrailError(
            f"CP2 guardrail: policy {policy!r} is B3. "
            "B3 is excluded from the CP2 ThresholdPolicy enum and must not "
...
_FULL_FRACTIONS: frozenset[float] = frozenset(CP2_MVP_FRACTIONS) | {0.05}
...
def assert_fractions_in_locked_grid(
    ...
    MVP grid: {0, 0.10, 0.20, 0.40}.
    Full scope adds 0.05 (conditional on CP2-T049 CONTINUE).
    Smoke/Stretch use the MVP grid.
    ...
    for f in fractions:
        if not any(abs(f - a) < 1e-9 for a in allowed):
            raise Cp2GuardrailError(
                f"CP2 guardrail: fraction {f} is not in the locked grid "
...
# ---------------------------------------------------------------------------
# MVP scope guardrail (SINGLE_CLIENT required)
# ---------------------------------------------------------------------------


def assert_mvp_requires_single_client(
    scale: ExperimentScale,
    target_scope_value: str,
) -> None:
    """Raise if MVP scale is paired with a non-SINGLE_CLIENT target scope.
    ...
    Raises:
        Cp2GuardrailError: If scale is MVP but scope is not SINGLE_CLIENT.
    """
    if scale == ExperimentScale.MVP and target_scope_value != "single_client":
        raise Cp2GuardrailError(
            f"CP2 guardrail: MVP scale requires SINGLE_CLIENT target scope; "
            f"got {target_scope_value!r}. Multi-client and all-client scopes "
            "are diagnostic-only and must not be used in MVP runs."
        )
