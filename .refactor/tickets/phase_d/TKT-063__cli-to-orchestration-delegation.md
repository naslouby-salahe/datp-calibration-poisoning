# TKT-063 — CLI to Orchestration Delegation
...
- Priority: P0
- Dependencies: TKT-061, TKT-048, TKT-037
...
Ensure CLI commands delegate to orchestration and do not own scientific work or manifest assembly. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-011. Its primary ownership boundary is narrower than the parent epic: cli to orchestration delegation.

## Problem Statement

Delegation can be audited independently from matrix preview and status behavior.
...
- Source location: src/datp/app/cli/poison.py run_bounded_sweep calls write_nbaiot_bounded_sweep_manifest after gate; preview and dry-run print stage info.
...
- Execution-time verification required: inspect the module responsible for cli to orchestration delegation; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Scientific behavior in CLI can diverge from tested orchestration. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for cli to orchestration delegation. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for cli to orchestration delegation if the decision gate authorizes changes.
...
- Production changes unrelated to cli to orchestration delegation.
...
- Intended canonical owner: the narrow package or module responsible for cli to orchestration delegation, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: cli to orchestration delegation. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify cli to orchestration delegation without changing unrelated behavior.
- architecture/import tests: verify cli to orchestration delegation without changing unrelated behavior.
- CLI tests: verify cli to orchestration delegation without changing unrelated behavior.
- manifest round-trip tests: verify cli to orchestration delegation without changing unrelated behavior.
- historical fixture tests: verify cli to orchestration delegation without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for cli to orchestration delegation.
...
- Update only terminology directly tied to cli to orchestration delegation.
...
- Prerequisites: TKT-061, TKT-048, TKT-037.
- Successors: TKT-064, TKT-065.
- Sibling tickets: TKT-060, TKT-061, TKT-064, TKT-065, TKT-062.
