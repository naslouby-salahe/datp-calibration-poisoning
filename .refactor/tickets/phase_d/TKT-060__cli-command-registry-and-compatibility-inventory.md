# TKT-060 — CLI Command Registry and Compatibility Inventory
...
- Blocks: TKT-061, TKT-063, TKT-062
...
Create a registry of actual Typer groups, command names, options, and compatibility aliases before CLI changes. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-011. Its primary ownership boundary is narrower than the parent epic: cli command registry and compatibility inventory.

## Problem Statement

Registry work is evidence-gathering and can close without code.
...
- Source location: src/datp/app/cli/__init__.py adds audit, checkpoint-protocol, config, poison, report groups and status/sweep commands; src/datp/app/cli/poison.py defines preview, dry-run, smoke, run-bounded-sweep, stages.
...
- Execution-time verification required: inspect the module responsible for cli command registry and compatibility inventory; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

CLI changes can break automation even if internals are correct. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for cli command registry and compatibility inventory. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for cli command registry and compatibility inventory if the decision gate authorizes changes.
...
- Production changes unrelated to cli command registry and compatibility inventory.
...
- Intended canonical owner: the narrow package or module responsible for cli command registry and compatibility inventory, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: cli command registry and compatibility inventory. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify cli command registry and compatibility inventory without changing unrelated behavior.
- architecture/import tests: verify cli command registry and compatibility inventory without changing unrelated behavior.
- CLI tests: verify cli command registry and compatibility inventory without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for cli command registry and compatibility inventory.
...
- Update only terminology directly tied to cli command registry and compatibility inventory.
...
- Prerequisites: TKT-045.
- Successors: TKT-061, TKT-063, TKT-062.
- Sibling tickets: TKT-061, TKT-063, TKT-064, TKT-065, TKT-062.
