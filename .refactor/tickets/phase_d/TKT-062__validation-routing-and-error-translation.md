# TKT-062 — Validation Routing and Error Translation
...
- Priority: P0
- Dependencies: TKT-060, TKT-052, TKT-054, TKT-040
- Blocks: TKT-068
...
Route CLI and orchestration validation to the correct owner and translate errors without weakening checks. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-011. Its primary ownership boundary is narrower than the parent epic: validation routing and error translation.

## Problem Statement

Validation routing is independently testable from status/resume.
...
- Source location: Validation appears in artifacts.existence, evaluation.artifact_validation, validation.provenance_gate, validation.score_manifest, reporting.validation, and CLI error handling.
...
- Execution-time verification required: inspect the module responsible for validation routing and error translation; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Misrouted validation can swallow protocol violations or produce unactionable CLI failures. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for validation routing and error translation. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for validation routing and error translation if the decision gate authorizes changes.
...
- Production changes unrelated to validation routing and error translation.
...
- Intended canonical owner: the narrow package or module responsible for validation routing and error translation, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: validation routing and error translation. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify validation routing and error translation without changing unrelated behavior.
- architecture/import tests: verify validation routing and error translation without changing unrelated behavior.
- CLI tests: verify validation routing and error translation without changing unrelated behavior.
- manifest round-trip tests: verify validation routing and error translation without changing unrelated behavior.
- historical fixture tests: verify validation routing and error translation without changing unrelated behavior.
- score-stage tests: verify validation routing and error translation without changing unrelated behavior.
- malformed input tests: verify validation routing and error translation without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for validation routing and error translation.
...
- Update only terminology directly tied to validation routing and error translation.
...
- Prerequisites: TKT-060, TKT-052, TKT-054, TKT-040.
- Successors: TKT-068.
- Sibling tickets: TKT-060, TKT-061, TKT-063, TKT-064, TKT-065.
