# TKT-064 — Canonical Matrix Creation and Preview Dry Run Consistency
...
- Priority: P0
- Dependencies: TKT-063, TKT-009, TKT-020
- Blocks: TKT-065
...
Make preview, dry-run, smoke, and bounded run use the same matrix definitions with mode-specific execution depth only. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-011. Its primary ownership boundary is narrower than the parent epic: canonical matrix creation and preview dry run consistency.

## Problem Statement

Matrix consistency is independently testable from resume and validation routing.
...
- Source location: src/datp/attacks/bounded_sweep_matrix.py enumerates bounded/full matrices; poison CLI dry-run currently prints stage configuration and does not enumerate exact cells.
...
- Execution-time verification required: inspect the module responsible for canonical matrix creation and preview dry run consistency; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Dry-run/run divergence can hide protocol drift until execution. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for canonical matrix creation and preview dry run consistency. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for canonical matrix creation and preview dry run consistency if the decision gate authorizes changes.
...
- Production changes unrelated to canonical matrix creation and preview dry run consistency.
...
- Intended canonical owner: the narrow package or module responsible for canonical matrix creation and preview dry run consistency, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: canonical matrix creation and preview dry run consistency. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify canonical matrix creation and preview dry run consistency without changing unrelated behavior.
- architecture/import tests: verify canonical matrix creation and preview dry run consistency without changing unrelated behavior.
- CLI tests: verify canonical matrix creation and preview dry run consistency without changing unrelated behavior.
- seed and pairing tests: verify canonical matrix creation and preview dry run consistency without changing unrelated behavior.
- policy semantic tests: verify canonical matrix creation and preview dry run consistency without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for canonical matrix creation and preview dry run consistency.
...
- Update only terminology directly tied to canonical matrix creation and preview dry run consistency.
...
- Prerequisites: TKT-063, TKT-009, TKT-020.
- Successors: TKT-065.
- Sibling tickets: TKT-060, TKT-061, TKT-063, TKT-065, TKT-062.
