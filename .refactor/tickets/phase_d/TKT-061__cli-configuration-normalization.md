# TKT-061 — CLI Configuration Normalization
...
- Priority: P0
- Dependencies: TKT-060, TKT-010
- Blocks: TKT-063, TKT-064
...
Normalize CLI inputs into typed config/enum objects without CLI-local protocol defaults. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-011. Its primary ownership boundary is narrower than the parent epic: cli configuration normalization.

## Problem Statement

Config normalization is distinct from orchestration delegation.
...
- Source location: src/datp/app/cli/poison.py defaults stage to ExperimentStage.NBAIOT_BOUNDED and reads stage configs; config CLI composes generic DatpConfig.
...
- Execution-time verification required: inspect the module responsible for cli configuration normalization; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

CLI-local defaults can bypass protocol ownership. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for cli configuration normalization. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for cli configuration normalization if the decision gate authorizes changes.
...
- Production changes unrelated to cli configuration normalization.
...
- Intended canonical owner: the narrow package or module responsible for cli configuration normalization, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: cli configuration normalization. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify cli configuration normalization without changing unrelated behavior.
- architecture/import tests: verify cli configuration normalization without changing unrelated behavior.
- CLI tests: verify cli configuration normalization without changing unrelated behavior.
- configuration tests: verify cli configuration normalization without changing unrelated behavior.
- seed and pairing tests: verify cli configuration normalization without changing unrelated behavior.
- policy semantic tests: verify cli configuration normalization without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for cli configuration normalization.
...
- Update only terminology directly tied to cli configuration normalization.
...
- Prerequisites: TKT-060, TKT-010.
- Successors: TKT-063, TKT-064.
- Sibling tickets: TKT-060, TKT-063, TKT-064, TKT-065, TKT-062.
