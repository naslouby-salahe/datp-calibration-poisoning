# TKT-065 — Status Command Lifecycle and Resume Gate Integration
...
- Priority: P0
- Dependencies: TKT-063, TKT-033, TKT-039
- Blocks: TKT-068
...
Make status and resume consume canonical lifecycle, manifest, and identity validation. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-011. Its primary ownership boundary is narrower than the parent epic: status command lifecycle and resume gate integration.

## Problem Statement

Status/resume behavior has distinct failure triggers from validation routing.
...
- Source location: src/datp/app/cli/status.py uses generic results_exist and RUN_ABORTED for generic experiment status; CP2 bounded sweep resume gate was not confirmed.
...
- Execution-time verification required: inspect the module responsible for status command lifecycle and resume gate integration; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Status can mark stale/corrupt CP2 artifacts complete. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for status command lifecycle and resume gate integration. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for status command lifecycle and resume gate integration if the decision gate authorizes changes.
...
- Production changes unrelated to status command lifecycle and resume gate integration.
...
- Intended canonical owner: the narrow package or module responsible for status command lifecycle and resume gate integration, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: status command lifecycle and resume gate integration. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify status command lifecycle and resume gate integration without changing unrelated behavior.
- architecture/import tests: verify status command lifecycle and resume gate integration without changing unrelated behavior.
- CLI tests: verify status command lifecycle and resume gate integration without changing unrelated behavior.
- lifecycle tests: verify status command lifecycle and resume gate integration without changing unrelated behavior.
- resume tests: verify status command lifecycle and resume gate integration without changing unrelated behavior.
- corruption tests: verify status command lifecycle and resume gate integration without changing unrelated behavior.
- manifest round-trip tests: verify status command lifecycle and resume gate integration without changing unrelated behavior.
- historical fixture tests: verify status command lifecycle and resume gate integration without changing unrelated behavior.
- seed and pairing tests: verify status command lifecycle and resume gate integration without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for status command lifecycle and resume gate integration.
...
- Update only terminology directly tied to status command lifecycle and resume gate integration.
...
- Prerequisites: TKT-063, TKT-033, TKT-039.
- Successors: TKT-068.
- Sibling tickets: TKT-060, TKT-061, TKT-063, TKT-064, TKT-062.
- Potential overlap risks: work under TKT-011 may touch nearby surfaces; use the parent epic child map before editing.
