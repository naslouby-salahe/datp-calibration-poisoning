# TKT-066 — Testsupport Fixture Taxonomy
...
- Priority: P2
- Dependencies: TKT-062
- Blocks: TKT-067, TKT-068
...
Classify reusable testsupport fixtures versus intentionally local scientific edge-case fixtures. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-012. Its primary ownership boundary is narrower than the parent epic: testsupport fixture taxonomy.

## Problem Statement

Fixture taxonomy is prerequisite cleanup evidence and can close no-code.
...
- Source location: src/datp/testsupport/synthetic_scores.py, smoke_harness.py, checkpoint_protocol.py exist; many attack tests import testsupport.synthetic_scores; local fixtures also exist in tests/unit.
...
- Execution-time verification required: inspect the module responsible for testsupport fixture taxonomy; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Over-centralized fixtures can erase policy-specific edge cases. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for testsupport fixture taxonomy. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for testsupport fixture taxonomy if the decision gate authorizes changes.
...
- Production changes unrelated to testsupport fixture taxonomy.
...
- Intended canonical owner: the narrow package or module responsible for testsupport fixture taxonomy, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: testsupport fixture taxonomy. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify testsupport fixture taxonomy without changing unrelated behavior.
- architecture/import tests: verify testsupport fixture taxonomy without changing unrelated behavior.
- policy semantic tests: verify testsupport fixture taxonomy without changing unrelated behavior.
- score-stage tests: verify testsupport fixture taxonomy without changing unrelated behavior.
- malformed input tests: verify testsupport fixture taxonomy without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for testsupport fixture taxonomy.
...
- Update only terminology directly tied to testsupport fixture taxonomy.
...
- Prerequisites: TKT-062.
- Successors: TKT-067, TKT-068.
- Sibling tickets: TKT-067, TKT-068, TKT-069, TKT-070.
