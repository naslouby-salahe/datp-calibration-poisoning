# TKT-067 — Runtime Testsupport Import Prohibition
...
Ensure production runtime modules do not import datp.testsupport except explicitly test-only smoke-fixture paths if approved. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-012. Its primary ownership boundary is narrower than the parent epic: runtime testsupport import prohibition.

## Problem Statement

Runtime import prohibition is a narrow package-boundary defect with independent rollback.
...
- Source location: rg confirmed src/datp/app/cli/checkpoint_protocol.py imports datp.testsupport.checkpoint_protocol inside a production CLI function; src/datp/testsupport/smoke_harness.py imports testsupport internally.
...
- Execution-time verification required: inspect the module responsible for runtime testsupport import prohibition; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Runtime dependency on testsupport breaks package hygiene and can ship test-only semantics. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for runtime testsupport import prohibition. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for runtime testsupport import prohibition if the decision gate authorizes changes.
...
- Production changes unrelated to runtime testsupport import prohibition.
...
- Intended canonical owner: the narrow package or module responsible for runtime testsupport import prohibition, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: runtime testsupport import prohibition. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify runtime testsupport import prohibition without changing unrelated behavior.
- architecture/import tests: verify runtime testsupport import prohibition without changing unrelated behavior.
- CLI tests: verify runtime testsupport import prohibition without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for runtime testsupport import prohibition.
...
- Update only terminology directly tied to runtime testsupport import prohibition.
...
- Prerequisites: TKT-066.
- Successors: TKT-068.
- Sibling tickets: TKT-066, TKT-068, TKT-069, TKT-070.
