# TKT-019 — B4 Recompute Call Path Normalization
...
Normalize compute_b4_pair callers around the validated B4 config boundary. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-003. Its primary ownership boundary is narrower than the parent epic: b4 recompute call path normalization.

## Problem Statement

Call-path normalization has a distinct public-interface and rollback boundary from validation and manifest work.
...
- Source location: src/datp/attacks/b4_recompute.py compute_b4_pair accepts scalar defaults k, n_init, max_iter, random_state, n_min, seed; src/datp/attacks/cell_runner.py recompute_pair calls compute_b4_pair without explicit B4 config.
...
- Execution-time verification required: inspect the module responsible for b4 recompute call path normalization; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Multiple scalar sources can create B4 parameter drift. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for b4 recompute call path normalization. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for b4 recompute call path normalization if the decision gate authorizes changes.
...
- Production changes unrelated to b4 recompute call path normalization.
...
- Intended canonical owner: the narrow package or module responsible for b4 recompute call path normalization, selected only after verification.
...
- unit tests: verify b4 recompute call path normalization without changing unrelated behavior.
- architecture/import tests: verify b4 recompute call path normalization without changing unrelated behavior.
- B4 determinism tests: verify b4 recompute call path normalization without changing unrelated behavior.
- policy semantic tests: verify b4 recompute call path normalization without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for b4 recompute call path normalization.
...
- Update only terminology directly tied to b4 recompute call path normalization.
...
- Prerequisites: TKT-016.
- Successors: TKT-051.
- Sibling tickets: TKT-014, TKT-016, TKT-021, TKT-024.
