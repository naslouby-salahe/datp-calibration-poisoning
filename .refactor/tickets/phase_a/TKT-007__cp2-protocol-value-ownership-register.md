# TKT-007 — CP2 Protocol Value Ownership Register
...
- Blocks: TKT-008, TKT-009, TKT-010, TKT-011, TKT-012
...
Classify every repeated CP2 value before any consolidation ticket changes code. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-002. Its primary ownership boundary is narrower than the parent epic: cp2 protocol value ownership register.

## Problem Statement

Value classification is prerequisite evidence, not the same implementation boundary as seed, matrix, YAML, enum, or constant migration.
...
- Source location: src/datp/artifacts/poison_names.py owns N_MIN, TAIL_MASS, threshold quantile, B4 constants, seed pools, and output root; src/datp/config/attack_config.py imports those values into typed CP2 config; src/datp/config/models.py has generic ThresholdConfig fields; src/datp/conf/config.yaml repeats n_min, q, B4 values, seeds, and unrelated 42/0.05/0.10 values.
...
- Execution-time verification required: inspect the module responsible for cp2 protocol value ownership register; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Premature consolidation can make protocol locks editable or merge equal literals with different scientific meaning. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for cp2 protocol value ownership register. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for cp2 protocol value ownership register if the decision gate authorizes changes.
...
- Production changes unrelated to cp2 protocol value ownership register.
...
- Intended canonical owner: the narrow package or module responsible for cp2 protocol value ownership register, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: cp2 protocol value ownership register. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify cp2 protocol value ownership register without changing unrelated behavior.
- architecture/import tests: verify cp2 protocol value ownership register without changing unrelated behavior.
- configuration tests: verify cp2 protocol value ownership register without changing unrelated behavior.
- manifest round-trip tests: verify cp2 protocol value ownership register without changing unrelated behavior.
- historical fixture tests: verify cp2 protocol value ownership register without changing unrelated behavior.
- seed and pairing tests: verify cp2 protocol value ownership register without changing unrelated behavior.
- B4 determinism tests: verify cp2 protocol value ownership register without changing unrelated behavior.
- policy semantic tests: verify cp2 protocol value ownership register without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for cp2 protocol value ownership register.
...
- Update only terminology directly tied to cp2 protocol value ownership register.
...
- Prerequisites: TKT-001, TKT-003.
- Successors: TKT-008, TKT-009, TKT-010, TKT-011, TKT-012.
- Sibling tickets: TKT-008, TKT-009, TKT-010, TKT-011, TKT-012.
