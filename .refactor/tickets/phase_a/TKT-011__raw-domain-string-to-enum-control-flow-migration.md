# TKT-011 — Raw Domain String to Enum Control Flow Migration
...
- Blocks: TKT-015, TKT-048, TKT-061
...
Replace internal raw control-flow strings with canonical enums while preserving boundary strings. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-002. Its primary ownership boundary is narrower than the parent epic: raw domain string to enum control flow migration.

## Problem Statement

Raw string cleanup is independently reversible and must not be mixed with value ownership decisions.
...
- Source location: src/datp/core/poison_enums.py provides canonical enums; src/datp/artifacts/poison_layout.py persists enum.value in paths; src/datp/scoring/schema.py SCORE_COLUMN is a persisted schema string and not an enum-control-flow candidate.
...
- Execution-time verification required: inspect the module responsible for raw domain string to enum control flow migration; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Internal string drift can bypass enum validation; boundary string changes can break artifacts. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for raw domain string to enum control flow migration. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for raw domain string to enum control flow migration if the decision gate authorizes changes.
...
- Production changes unrelated to raw domain string to enum control flow migration.
...
- API-PUBLIC: Public imports, re-exports, and programmatic entry points remain compatible unless explicitly migrated. Status: preserved unchanged unless an explicitly additive compatibility path is required.
...
- Intended canonical owner: the narrow package or module responsible for raw domain string to enum control flow migration, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: raw domain string to enum control flow migration. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify raw domain string to enum control flow migration without changing unrelated behavior.
- architecture/import tests: verify raw domain string to enum control flow migration without changing unrelated behavior.
- CLI tests: verify raw domain string to enum control flow migration without changing unrelated behavior.
- artifact path tests: verify raw domain string to enum control flow migration without changing unrelated behavior.
- manifest round-trip tests: verify raw domain string to enum control flow migration without changing unrelated behavior.
- historical fixture tests: verify raw domain string to enum control flow migration without changing unrelated behavior.
- policy semantic tests: verify raw domain string to enum control flow migration without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for raw domain string to enum control flow migration.
...
- Update only terminology directly tied to raw domain string to enum control flow migration.
...
- Successors: TKT-015, TKT-048, TKT-061.
- Sibling tickets: TKT-007, TKT-008, TKT-009, TKT-010, TKT-012.
