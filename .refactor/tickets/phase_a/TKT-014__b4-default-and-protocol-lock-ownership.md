# TKT-014 — B4 Default and Protocol Lock Ownership
...
- Priority: P0
- Dependencies: TKT-007, TKT-012, TKT-003
- Blocks: TKT-016, TKT-019
...
Assign canonical CP2 ownership for B4 K, n_init, max_iter, and random_state. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-003. Its primary ownership boundary is narrower than the parent epic: b4 default and protocol lock ownership.

## Problem Statement

B4 defaults are separable from recompute interface normalization and manifest provenance.
...
- Source location: src/datp/artifacts/poison_names.py defines B4_K=3, B4_N_INIT=10, B4_MAX_ITER=300, B4_RANDOM_STATE=42; src/datp/config/attack_config.py B4ClusterConfig imports them and validates k==3; src/datp/conf/config.yaml repeats b4 values in generic threshold config.
...
- Execution-time verification required: inspect the module responsible for b4 default and protocol lock ownership; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

B4 default drift changes clustering and all B4 comparisons. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for b4 default and protocol lock ownership. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for b4 default and protocol lock ownership if the decision gate authorizes changes.
...
- Production changes unrelated to b4 default and protocol lock ownership.
...
- Intended canonical owner: the narrow package or module responsible for b4 default and protocol lock ownership, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: b4 default and protocol lock ownership. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify b4 default and protocol lock ownership without changing unrelated behavior.
- architecture/import tests: verify b4 default and protocol lock ownership without changing unrelated behavior.
- configuration tests: verify b4 default and protocol lock ownership without changing unrelated behavior.
- B4 determinism tests: verify b4 default and protocol lock ownership without changing unrelated behavior.
- policy semantic tests: verify b4 default and protocol lock ownership without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for b4 default and protocol lock ownership.
...
- Update only terminology directly tied to b4 default and protocol lock ownership.
...
- Prerequisites: TKT-007, TKT-012, TKT-003.
- Successors: TKT-016, TKT-019.
- Sibling tickets: TKT-016, TKT-019, TKT-021, TKT-024.
