# TKT-024 — B4 Determinism and Decomposition Regression Protection
...
- Priority: P0
- Dependencies: TKT-019, TKT-003
- Blocks: TKT-051, TKT-055
...
Harden tests around deterministic B4 behavior and decomposition identity after B4 flow changes. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-003. Its primary ownership boundary is narrower than the parent epic: b4 determinism and decomposition regression protection.

## Problem Statement

B4 regression protection is test-focused and distinct from runtime ownership changes.
...
- Source location: src/datp/attacks/b4_recompute.py documents raw cluster labels are never compared and delta_tau_total = delta_tau_agg + delta_tau_churn; tests/integration/attacks/test_smoke_harness.py asserts B4 decomposition identity and K lock.
...
- Execution-time verification required: inspect the module responsible for b4 determinism and decomposition regression protection; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

B4 decomposition drift can preserve top-level thresholds while corrupting scientific explanation. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for b4 determinism and decomposition regression protection. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for b4 determinism and decomposition regression protection if the decision gate authorizes changes.
...
- Production changes unrelated to b4 determinism and decomposition regression protection.
...
- Intended canonical owner: the narrow package or module responsible for b4 determinism and decomposition regression protection, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: b4 determinism and decomposition regression protection. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify b4 determinism and decomposition regression protection without changing unrelated behavior.
- architecture/import tests: verify b4 determinism and decomposition regression protection without changing unrelated behavior.
- B4 determinism tests: verify b4 determinism and decomposition regression protection without changing unrelated behavior.
- policy semantic tests: verify b4 determinism and decomposition regression protection without changing unrelated behavior.
- score-stage tests: verify b4 determinism and decomposition regression protection without changing unrelated behavior.
- malformed input tests: verify b4 determinism and decomposition regression protection without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for b4 determinism and decomposition regression protection.
...
- Update only terminology directly tied to b4 determinism and decomposition regression protection.
...
- Prerequisites: TKT-019, TKT-003.
- Successors: TKT-051, TKT-055.
- Sibling tickets: TKT-014, TKT-016, TKT-019, TKT-021.
