# TKT-022 — Identity to Path Projection
...
- Priority: P0
- Dependencies: TKT-018, TKT-004
- Blocks: TKT-027, TKT-036
...
Make CP2 path projection derive from canonical identity while preserving exact current path strings. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-004. Its primary ownership boundary is narrower than the parent epic: identity to path projection.

## Problem Statement

Path projection is an artifact compatibility migration, separate from semantic identity and manifest projection.
...
- Source location: src/datp/artifacts/poison_layout.py PoisonLayout.run_dir constructs paths from CellId fields and fixed segment format; tests/unit/artifacts/test_poison_layout.py asserts scope, train, poison, and output root segments.
...
- Execution-time verification required: inspect the module responsible for identity to path projection; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Path projection errors can overwrite or merge results. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for identity to path projection. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for identity to path projection if the decision gate authorizes changes.
...
- Production changes unrelated to identity to path projection.
...
- Intended canonical owner: the narrow package or module responsible for identity to path projection, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: identity to path projection. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify identity to path projection without changing unrelated behavior.
- architecture/import tests: verify identity to path projection without changing unrelated behavior.
- artifact path tests: verify identity to path projection without changing unrelated behavior.
- seed and pairing tests: verify identity to path projection without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for identity to path projection.
...
- Update only terminology directly tied to identity to path projection.
...
- Prerequisites: TKT-018, TKT-004.
- Successors: TKT-027, TKT-036.
- Sibling tickets: TKT-015, TKT-018, TKT-023, TKT-020.
