# TKT-009 — Fraction Grid and Policy Matrix Ownership
...
- Priority: P0
- Dependencies: TKT-007, TKT-003
- Blocks: TKT-015, TKT-064
...
Make matrix condition ownership explicit for policies, sources, objectives, fractions, and target scope. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-002. Its primary ownership boundary is narrower than the parent epic: fraction grid and policy matrix ownership.

## Problem Statement

Matrix ownership is independently testable from seed pool propagation and raw enum cleanup.
...
- Source location: src/datp/core/poison_enums.py defines DEFAULT_POLICIES, BOUNDED_SWEEP_FRACTIONS, FULL_SWEEP_FRACTIONS, BOUNDED_SWEEP_SOURCES; src/datp/attacks/bounded_sweep_matrix.py enumerates the single-victim matrix over those tuples; src/datp/attacks/source_strategies.py derives objective_for_source.
...
- Execution-time verification required: inspect the module responsible for fraction grid and policy matrix ownership; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Matrix drift changes condition count, B3 exclusion, or bounded/full distinction. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for fraction grid and policy matrix ownership. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for fraction grid and policy matrix ownership if the decision gate authorizes changes.
...
- Production changes unrelated to fraction grid and policy matrix ownership.
...
- Intended canonical owner: the narrow package or module responsible for fraction grid and policy matrix ownership, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: fraction grid and policy matrix ownership. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify fraction grid and policy matrix ownership without changing unrelated behavior.
- architecture/import tests: verify fraction grid and policy matrix ownership without changing unrelated behavior.
- configuration tests: verify fraction grid and policy matrix ownership without changing unrelated behavior.
- seed and pairing tests: verify fraction grid and policy matrix ownership without changing unrelated behavior.
- policy semantic tests: verify fraction grid and policy matrix ownership without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for fraction grid and policy matrix ownership.
...
- Update only terminology directly tied to fraction grid and policy matrix ownership.
...
- Prerequisites: TKT-007, TKT-003.
- Successors: TKT-015, TKT-064.
- Sibling tickets: TKT-007, TKT-008, TKT-010, TKT-011, TKT-012.
