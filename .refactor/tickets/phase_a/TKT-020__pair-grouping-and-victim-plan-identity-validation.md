# TKT-020 — Pair Grouping and Victim Plan Identity Validation
...
Validate that result grouping preserves victim or victim-plan identity across clean and poisoned pairs. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-004. Its primary ownership boundary is narrower than the parent epic: pair grouping and victim plan identity validation.

## Problem Statement

Pair/victim validation can be added independently after identity owners exist.
...
- Source location: src/datp/attacks/cell_runner.py has single and multi victim injection outcomes; src/datp/attacks/bounded_sweep_cell.py SweepCellResult stores victim_id, source, fraction, training_seed, poisoning_seed; aggregate rows persist victim_id.
...
- Execution-time verification required: inspect the module responsible for pair grouping and victim plan identity validation; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Wrong victim grouping can turn spillover/blast metrics into false evidence. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for pair grouping and victim plan identity validation. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for pair grouping and victim plan identity validation if the decision gate authorizes changes.
...
- Production changes unrelated to pair grouping and victim plan identity validation.
...
- Intended canonical owner: the narrow package or module responsible for pair grouping and victim plan identity validation, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: pair grouping and victim plan identity validation. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify pair grouping and victim plan identity validation without changing unrelated behavior.
- architecture/import tests: verify pair grouping and victim plan identity validation without changing unrelated behavior.
- manifest round-trip tests: verify pair grouping and victim plan identity validation without changing unrelated behavior.
- historical fixture tests: verify pair grouping and victim plan identity validation without changing unrelated behavior.
- seed and pairing tests: verify pair grouping and victim plan identity validation without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for pair grouping and victim plan identity validation.
...
- Update only terminology directly tied to pair grouping and victim plan identity validation.
...
- Prerequisites: TKT-018.
- Successors: TKT-064, TKT-065.
- Sibling tickets: TKT-015, TKT-018, TKT-022, TKT-023.
