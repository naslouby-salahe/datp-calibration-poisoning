# TKT-021 — B4 Manifest Provenance Compatibility
...
Ensure persisted CP2 manifests record or validate effective B4 configuration without breaking existing schemas. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-003. Its primary ownership boundary is narrower than the parent epic: b4 manifest provenance compatibility.

## Problem Statement

Manifest provenance is a serialization boundary and must be separate from recompute interface work.
...
- Source location: src/datp/attacks/run_manifest.py RunManifest records policy, provenance, seed_record, and mu_flag_threshold; src/datp/attacks/bounded_sweep_manifest.py BoundedSweepManifest records policies and result rows; source evidence did not show explicit B4 config fields in those models.
...
- Execution-time verification required: inspect the module responsible for b4 manifest provenance compatibility; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Unrecorded B4 config makes B4 artifacts hard to audit; incompatible fields break historical readers. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for b4 manifest provenance compatibility. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for b4 manifest provenance compatibility if the decision gate authorizes changes.
...
- Production changes unrelated to b4 manifest provenance compatibility.
...
- Intended canonical owner: the narrow package or module responsible for b4 manifest provenance compatibility, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: b4 manifest provenance compatibility. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify b4 manifest provenance compatibility without changing unrelated behavior.
- architecture/import tests: verify b4 manifest provenance compatibility without changing unrelated behavior.
- artifact path tests: verify b4 manifest provenance compatibility without changing unrelated behavior.
- manifest round-trip tests: verify b4 manifest provenance compatibility without changing unrelated behavior.
- historical fixture tests: verify b4 manifest provenance compatibility without changing unrelated behavior.
- B4 determinism tests: verify b4 manifest provenance compatibility without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for b4 manifest provenance compatibility.
...
- Update only terminology directly tied to b4 manifest provenance compatibility.
...
- Prerequisites: TKT-016, TKT-005.
- Successors: TKT-034, TKT-037.
- Sibling tickets: TKT-014, TKT-016, TKT-019, TKT-024.
