# TKT-010 — YAML to Typed CP2 Effective Configuration Validation
...
Prevent YAML/generic config from silently overriding CP2 protocol locks. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-002. Its primary ownership boundary is narrower than the parent epic: yaml to typed cp2 effective configuration validation.

## Problem Statement

YAML validation has a distinct compatibility surface from code constants and matrix enumeration.
...
- Source location: src/datp/conf/config.yaml repeats threshold.n_min, threshold.q, b4_* values, experiment.seeds, and unrelated statistics/reporting seeds; src/datp/config/models.py validates generic DatpConfig; src/datp/config/attack_config.py validates CalibrationPoisoningConfig separately.
...
- Execution-time verification required: inspect the module responsible for yaml to typed cp2 effective configuration validation; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Editable YAML defaults can change CP2 protocol behavior without changing source constants. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for yaml to typed cp2 effective configuration validation. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for yaml to typed cp2 effective configuration validation if the decision gate authorizes changes.
...
- Production changes unrelated to yaml to typed cp2 effective configuration validation.
...
- CFG-YAML: YAML keys and resolved effective configuration behavior remain compatible and cannot silently override CP2 protocol locks. Status: preserved unchanged unless an explicitly additive compatibility path is required.
...
- Intended canonical owner: the narrow package or module responsible for yaml to typed cp2 effective configuration validation, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: yaml to typed cp2 effective configuration validation. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- Configuration: confirmed relevant.
...
- YAML input: preserve keys and prevent silent CP2 lock overrides.
...
- unit tests: verify yaml to typed cp2 effective configuration validation without changing unrelated behavior.
- architecture/import tests: verify yaml to typed cp2 effective configuration validation without changing unrelated behavior.
- configuration tests: verify yaml to typed cp2 effective configuration validation without changing unrelated behavior.
- manifest round-trip tests: verify yaml to typed cp2 effective configuration validation without changing unrelated behavior.
- historical fixture tests: verify yaml to typed cp2 effective configuration validation without changing unrelated behavior.
- seed and pairing tests: verify yaml to typed cp2 effective configuration validation without changing unrelated behavior.
- B4 determinism tests: verify yaml to typed cp2 effective configuration validation without changing unrelated behavior.
- policy semantic tests: verify yaml to typed cp2 effective configuration validation without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for yaml to typed cp2 effective configuration validation.
...
- Update only terminology directly tied to yaml to typed cp2 effective configuration validation.
...
- Prerequisites: TKT-007.
- Successors: TKT-039, TKT-061.
- Sibling tickets: TKT-007, TKT-008, TKT-009, TKT-011, TKT-012.
- Potential overlap risks: work under TKT-002 may touch nearby surfaces; use the parent epic child map before editing.
