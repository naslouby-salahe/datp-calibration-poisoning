# TKT-016 — B4 Typed Configuration Validation Boundary
...
- Priority: P0
- Dependencies: TKT-014, TKT-010
- Blocks: TKT-019, TKT-021, TKT-051
...
Make B4ClusterConfig the validated CP2 boundary or document the already-equivalent boundary. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-003. Its primary ownership boundary is narrower than the parent epic: b4 typed configuration validation boundary.

## Problem Statement

Config validation can be implemented, tested, audited, and rolled back separately from recompute call signatures.
...
- Source location: src/datp/config/attack_config.py B4ClusterConfig is frozen and forbids extra fields; src/datp/config/models.py ThresholdConfig is generic and includes b4_* fields; tests/unit/config/test_attack_config.py and test_models.py exist.
...
- Execution-time verification required: inspect the module responsible for b4 typed configuration validation boundary; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Invalid or competing B4 config can silently enter the CP2 run path. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for b4 typed configuration validation boundary. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for b4 typed configuration validation boundary if the decision gate authorizes changes.
...
- Production changes unrelated to b4 typed configuration validation boundary.
...
- Intended canonical owner: the narrow package or module responsible for b4 typed configuration validation boundary, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: b4 typed configuration validation boundary. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify b4 typed configuration validation boundary without changing unrelated behavior.
- architecture/import tests: verify b4 typed configuration validation boundary without changing unrelated behavior.
- configuration tests: verify b4 typed configuration validation boundary without changing unrelated behavior.
- B4 determinism tests: verify b4 typed configuration validation boundary without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for b4 typed configuration validation boundary.
...
- Update only terminology directly tied to b4 typed configuration validation boundary.
...
- Prerequisites: TKT-014, TKT-010.
- Successors: TKT-019, TKT-021, TKT-051.
- Sibling tickets: TKT-014, TKT-019, TKT-021, TKT-024.
- Potential overlap risks: work under TKT-003 may touch nearby surfaces; use the parent epic child map before editing.
