# TKT-015 — CP2 Condition Identity Semantic Owner
...
- Priority: P0
- Dependencies: TKT-009, TKT-011
- Blocks: TKT-018, TKT-022, TKT-023
...
Define the condition-level CP2 semantic identity and keep boundary projections separate. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-004. Its primary ownership boundary is narrower than the parent epic: cp2 condition identity semantic owner.

## Problem Statement

Condition identity excludes run/victim/seed details and can be validated independently.
...
- Source location: src/datp/artifacts/poison_layout.py CellId contains scale, dataset, policy, objective, source, fraction, target_scope, training_seed, poisoning_seed; src/datp/attacks/bounded_sweep_matrix.py SweepCellSpec contains training_seed, poisoning_seed, victim_id, policy, source, fraction, target_scope.
...
- Execution-time verification required: inspect the module responsible for cp2 condition identity semantic owner; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Ambiguous condition identity causes path, manifest, and result grouping drift. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for cp2 condition identity semantic owner. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for cp2 condition identity semantic owner if the decision gate authorizes changes.
...
- Production changes unrelated to cp2 condition identity semantic owner.
...
- Intended canonical owner: the narrow package or module responsible for cp2 condition identity semantic owner, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: cp2 condition identity semantic owner. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify cp2 condition identity semantic owner without changing unrelated behavior.
- architecture/import tests: verify cp2 condition identity semantic owner without changing unrelated behavior.
- artifact path tests: verify cp2 condition identity semantic owner without changing unrelated behavior.
- manifest round-trip tests: verify cp2 condition identity semantic owner without changing unrelated behavior.
- historical fixture tests: verify cp2 condition identity semantic owner without changing unrelated behavior.
- seed and pairing tests: verify cp2 condition identity semantic owner without changing unrelated behavior.
- policy semantic tests: verify cp2 condition identity semantic owner without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for cp2 condition identity semantic owner.
...
- Update only terminology directly tied to cp2 condition identity semantic owner.
...
- Prerequisites: TKT-009, TKT-011.
- Successors: TKT-018, TKT-022, TKT-023.
- Sibling tickets: TKT-018, TKT-022, TKT-023, TKT-020.
