# TKT-018 — CP2 Run Identity and Seed Pairing Owner
...
- Priority: P0
- Dependencies: TKT-015, TKT-008
- Blocks: TKT-022, TKT-020, TKT-039
...
Define run identity around condition, training seed, poisoning seed, and victim/victim plan. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-004. Its primary ownership boundary is narrower than the parent epic: cp2 run identity and seed pairing owner.

## Problem Statement

Run identity is independently testable from path and manifest projections.
...
- Source location: src/datp/attacks/bounded_sweep_matrix.py pairs TRAINING_SEEDS and POISONING_SEEDS with zip(strict=True); src/datp/core/seed_sequence.py SeedRecord carries training_seed, poisoning_seed, client_idx, scope_idx; src/datp/attacks/bounded_sweep_manifest.py rows persist victim_id and seed_record.
...
- Execution-time verification required: inspect the module responsible for cp2 run identity and seed pairing owner; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Pair drift can combine clean and poisoned outputs from different runs. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for cp2 run identity and seed pairing owner. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for cp2 run identity and seed pairing owner if the decision gate authorizes changes.
...
- Production changes unrelated to cp2 run identity and seed pairing owner.
...
- Intended canonical owner: the narrow package or module responsible for cp2 run identity and seed pairing owner, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: cp2 run identity and seed pairing owner. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify cp2 run identity and seed pairing owner without changing unrelated behavior.
- architecture/import tests: verify cp2 run identity and seed pairing owner without changing unrelated behavior.
- manifest round-trip tests: verify cp2 run identity and seed pairing owner without changing unrelated behavior.
- historical fixture tests: verify cp2 run identity and seed pairing owner without changing unrelated behavior.
- seed and pairing tests: verify cp2 run identity and seed pairing owner without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for cp2 run identity and seed pairing owner.
...
- Update only terminology directly tied to cp2 run identity and seed pairing owner.
...
- Prerequisites: TKT-015, TKT-008.
- Successors: TKT-022, TKT-020, TKT-039.
- Sibling tickets: TKT-015, TKT-022, TKT-023, TKT-020.
