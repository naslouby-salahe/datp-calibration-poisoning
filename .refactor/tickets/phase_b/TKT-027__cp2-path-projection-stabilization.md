# TKT-027 — CP2 Path Projection Stabilization
...
- Blocks: TKT-029, TKT-036, TKT-034
...
Stabilize CP2 path construction as an identity projection with exact compatibility snapshots. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-005. Its primary ownership boundary is narrower than the parent epic: cp2 path projection stabilization.

## Problem Statement

Path stabilization is independently reversible from lifecycle and resume behavior.
...
- Source location: src/datp/artifacts/poison_layout.py is documented as canonical run-path builder; tests/unit/artifacts/test_poison_layout.py asserts current path segments and constants.
...
- Execution-time verification required: inspect the module responsible for cp2 path projection stabilization; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Ad hoc path construction can create duplicate or undiscoverable output locations. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for cp2 path projection stabilization. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for cp2 path projection stabilization if the decision gate authorizes changes.
...
- Production changes unrelated to cp2 path projection stabilization.
...
- Intended canonical owner: the narrow package or module responsible for cp2 path projection stabilization, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: cp2 path projection stabilization. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify cp2 path projection stabilization without changing unrelated behavior.
- architecture/import tests: verify cp2 path projection stabilization without changing unrelated behavior.
- artifact path tests: verify cp2 path projection stabilization without changing unrelated behavior.
- manifest round-trip tests: verify cp2 path projection stabilization without changing unrelated behavior.
- historical fixture tests: verify cp2 path projection stabilization without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for cp2 path projection stabilization.
...
- Update only terminology directly tied to cp2 path projection stabilization.
...
- Prerequisites: TKT-022.
- Successors: TKT-029, TKT-036, TKT-034.
- Sibling tickets: TKT-025, TKT-029, TKT-033, TKT-036.
