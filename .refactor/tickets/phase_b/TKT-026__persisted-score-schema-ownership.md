# TKT-026 — Persisted Score Schema Ownership
...
- Priority: P1
- Dependencies: TKT-006, TKT-012
- Blocks: TKT-028, TKT-032
...
Confirm scoring.schema is the sole owner of persisted score column vocabulary and manifest schema constants. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-007. Its primary ownership boundary is narrower than the parent epic: persisted score schema ownership.

## Problem Statement

Schema ownership is independent from loading paths and CP2 runtime adaptation.
...
- Source location: src/datp/scoring/schema.py defines SCORE_COLUMN="reconstruction_error", SCORING_MANIFEST_SCHEMA_VERSION, ScoringManifestStatus; tests/integration/scoring/test_score_artifacts.py asserts the column and dtype.
...
- Execution-time verification required: inspect the module responsible for persisted score schema ownership; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Schema drift breaks Parquet loading and score-stage safety. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for persisted score schema ownership. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for persisted score schema ownership if the decision gate authorizes changes.
...
- Production changes unrelated to persisted score schema ownership.
...
- Intended canonical owner: the narrow package or module responsible for persisted score schema ownership, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: persisted score schema ownership. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify persisted score schema ownership without changing unrelated behavior.
- architecture/import tests: verify persisted score schema ownership without changing unrelated behavior.
- manifest round-trip tests: verify persisted score schema ownership without changing unrelated behavior.
- historical fixture tests: verify persisted score schema ownership without changing unrelated behavior.
- score-stage tests: verify persisted score schema ownership without changing unrelated behavior.
- malformed input tests: verify persisted score schema ownership without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for persisted score schema ownership.
...
- Update only terminology directly tied to persisted score schema ownership.
...
- Prerequisites: TKT-006, TKT-012.
- Successors: TKT-028, TKT-032.
- Sibling tickets: TKT-028, TKT-032, TKT-035, TKT-038, TKT-040.
