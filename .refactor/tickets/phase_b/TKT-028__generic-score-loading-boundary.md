# TKT-028 — Generic Score Loading Boundary
...
- Priority: P1
- Dependencies: TKT-026
- Blocks: TKT-032, TKT-044
...
Make generic score loading own persisted Parquet read/validation behavior. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-007. Its primary ownership boundary is narrower than the parent epic: generic score loading boundary.

## Problem Statement

Generic loading can be fixed without touching CP2 runtime containers.
...
- Source location: src/datp/scoring/loading.py read_score_column calls validate_score_artifact and reads SCORE_COLUMN; load_parquets_from_dir loads *.parquet; ScoreProvider resolves ScoringStage paths.
...
- Execution-time verification required: inspect the module responsible for generic score loading boundary; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Duplicate generic loaders create inconsistent validation and error behavior. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for generic score loading boundary. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for generic score loading boundary if the decision gate authorizes changes.
...
- Production changes unrelated to generic score loading boundary.
...
- Intended canonical owner: the narrow package or module responsible for generic score loading boundary, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: generic score loading boundary. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify generic score loading boundary without changing unrelated behavior.
- architecture/import tests: verify generic score loading boundary without changing unrelated behavior.
- artifact path tests: verify generic score loading boundary without changing unrelated behavior.
- score-stage tests: verify generic score loading boundary without changing unrelated behavior.
- malformed input tests: verify generic score loading boundary without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for generic score loading boundary.
...
- Update only terminology directly tied to generic score loading boundary.
...
- Prerequisites: TKT-026.
- Successors: TKT-032, TKT-044.
- Sibling tickets: TKT-026, TKT-032, TKT-035, TKT-038, TKT-040.
