# TKT-037 — Aggregate Bounded Sweep Manifest Construction Boundary
...
- Priority: P1
- Dependencies: TKT-031, TKT-036
- Blocks: TKT-047, TKT-063
...
Make bounded aggregate manifest assembly and n_cells/result consistency explicit. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-006. Its primary ownership boundary is narrower than the parent epic: aggregate bounded sweep manifest construction boundary.

## Problem Statement

Aggregate schema and per-run schema have different lifecycles and rollback triggers.
...
- Source location: src/datp/attacks/bounded_sweep_manifest.py validates n_cells, paired seed pool lengths, and mu_flag keys; src/datp/attacks/bounded_sweep_run.py builds BoundedSweepManifest and writes nbaiot_bounded_sweep_manifest.json.
...
- Execution-time verification required: inspect the module responsible for aggregate bounded sweep manifest construction boundary; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Aggregate manifest drift corrupts matrix completeness and reporting inputs. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for aggregate bounded sweep manifest construction boundary. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for aggregate bounded sweep manifest construction boundary if the decision gate authorizes changes.
...
- Production changes unrelated to aggregate bounded sweep manifest construction boundary.
...
- Intended canonical owner: the narrow package or module responsible for aggregate bounded sweep manifest construction boundary, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: aggregate bounded sweep manifest construction boundary. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify aggregate bounded sweep manifest construction boundary without changing unrelated behavior.
- architecture/import tests: verify aggregate bounded sweep manifest construction boundary without changing unrelated behavior.
- manifest round-trip tests: verify aggregate bounded sweep manifest construction boundary without changing unrelated behavior.
- historical fixture tests: verify aggregate bounded sweep manifest construction boundary without changing unrelated behavior.
- seed and pairing tests: verify aggregate bounded sweep manifest construction boundary without changing unrelated behavior.
- B4 determinism tests: verify aggregate bounded sweep manifest construction boundary without changing unrelated behavior.
- policy semantic tests: verify aggregate bounded sweep manifest construction boundary without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for aggregate bounded sweep manifest construction boundary.
...
- Update only terminology directly tied to aggregate bounded sweep manifest construction boundary.
...
- Prerequisites: TKT-031, TKT-036.
- Successors: TKT-047, TKT-063.
- Sibling tickets: TKT-031, TKT-034, TKT-030, TKT-039.
