# TKT-038 — Reservoir Source Enforcement
...
Enforce that reservoirs use victim-local benign calibration scores only. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-007. Its primary ownership boundary is narrower than the parent epic: reservoir source enforcement.

## Problem Statement

Reservoir enforcement can be validated independently from score Parquet loading.
...
- Source location: src/datp/attacks/reservoir.py builds reservoirs from clean_cal only; src/datp/attacks/guardrails.py rejects labels containing test/train; src/datp/attacks/cell_runner.py select_reservoir receives victim_clean.
...
- Execution-time verification required: inspect the module responsible for reservoir source enforcement; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Reservoir contamination expands the threat model beyond calibration poisoning. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for reservoir source enforcement. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for reservoir source enforcement if the decision gate authorizes changes.
...
- Production changes unrelated to reservoir source enforcement.
...
- Intended canonical owner: the narrow package or module responsible for reservoir source enforcement, selected only after verification.
...
- unit tests: verify reservoir source enforcement without changing unrelated behavior.
- architecture/import tests: verify reservoir source enforcement without changing unrelated behavior.
- score-stage tests: verify reservoir source enforcement without changing unrelated behavior.
- malformed input tests: verify reservoir source enforcement without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for reservoir source enforcement.
...
- Update only terminology directly tied to reservoir source enforcement.
...
- Prerequisites: TKT-032, TKT-006.
- Successors: TKT-053.
- Sibling tickets: TKT-026, TKT-028, TKT-032, TKT-035, TKT-040.
