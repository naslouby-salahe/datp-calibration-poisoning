# TKT-036 — Artifact Discovery and Path Uniqueness Compatibility
...
Validate discovery surfaces and uniqueness for current and historical CP2 artifacts. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-005. Its primary ownership boundary is narrower than the parent epic: artifact discovery and path uniqueness compatibility.

## Problem Statement

Discovery/uniqueness can be accepted or rolled back without altering lifecycle state code.
...
- Source location: src/datp/artifacts/poison_layout.py aggregate manifest path omits victim_id; bounded_sweep_manifest.py documents one aggregate file for locked matrix; CellId has no victim_id.
...
- Execution-time verification required: inspect the module responsible for artifact discovery and path uniqueness compatibility; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Path collisions can overwrite distinct victim runs or duplicate report rows. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for artifact discovery and path uniqueness compatibility. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for artifact discovery and path uniqueness compatibility if the decision gate authorizes changes.
...
- Production changes unrelated to artifact discovery and path uniqueness compatibility.
...
- Intended canonical owner: the narrow package or module responsible for artifact discovery and path uniqueness compatibility, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: artifact discovery and path uniqueness compatibility. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify artifact discovery and path uniqueness compatibility without changing unrelated behavior.
- architecture/import tests: verify artifact discovery and path uniqueness compatibility without changing unrelated behavior.
- artifact path tests: verify artifact discovery and path uniqueness compatibility without changing unrelated behavior.
- lifecycle tests: verify artifact discovery and path uniqueness compatibility without changing unrelated behavior.
- resume tests: verify artifact discovery and path uniqueness compatibility without changing unrelated behavior.
- corruption tests: verify artifact discovery and path uniqueness compatibility without changing unrelated behavior.
- manifest round-trip tests: verify artifact discovery and path uniqueness compatibility without changing unrelated behavior.
- historical fixture tests: verify artifact discovery and path uniqueness compatibility without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for artifact discovery and path uniqueness compatibility.
...
- Update only terminology directly tied to artifact discovery and path uniqueness compatibility.
...
- Prerequisites: TKT-027, TKT-020.
- Successors: TKT-065.
- Sibling tickets: TKT-027, TKT-025, TKT-029, TKT-033.
