# TKT-033 — Resume Eligibility Validation
...
Define when a CP2 run may resume, skip, rerun, or block. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-005. Its primary ownership boundary is narrower than the parent epic: resume eligibility validation.

## Problem Statement

Resume gate behavior is independently auditable and must not be hidden inside CLI cleanup.
...
- Source location: src/datp/artifacts/lifecycle.py provides marker state; src/datp/attacks/run_logger.py refuses manifest emission without mu_flag_threshold; existing source evidence did not show a full CP2 resume gate.
...
- Execution-time verification required: inspect the module responsible for resume eligibility validation; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Unsafe resume can mix old and new protocol outputs. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for resume eligibility validation. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for resume eligibility validation if the decision gate authorizes changes.
...
- Production changes unrelated to resume eligibility validation.
...
- Intended canonical owner: the narrow package or module responsible for resume eligibility validation, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: resume eligibility validation. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify resume eligibility validation without changing unrelated behavior.
- architecture/import tests: verify resume eligibility validation without changing unrelated behavior.
- CLI tests: verify resume eligibility validation without changing unrelated behavior.
- lifecycle tests: verify resume eligibility validation without changing unrelated behavior.
- resume tests: verify resume eligibility validation without changing unrelated behavior.
- corruption tests: verify resume eligibility validation without changing unrelated behavior.
- manifest round-trip tests: verify resume eligibility validation without changing unrelated behavior.
- historical fixture tests: verify resume eligibility validation without changing unrelated behavior.
- seed and pairing tests: verify resume eligibility validation without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for resume eligibility validation.
...
- Update only terminology directly tied to resume eligibility validation.
...
- Prerequisites: TKT-025, TKT-029.
- Successors: TKT-065.
- Sibling tickets: TKT-027, TKT-025, TKT-029, TKT-036.
