# TKT-039 — Manifest Post Read and Historical Compatibility Validation
...
- Priority: P1
- Dependencies: TKT-034, TKT-037, TKT-033
- Blocks: TKT-065
...
Validate loaded manifests against current protocol and compatibility rules. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-006. Its primary ownership boundary is narrower than the parent epic: manifest post read and historical compatibility validation.

## Problem Statement

Read-time compatibility has different failure boundaries from write-time conversion.
...
- Source location: src/datp/attacks/run_logger.py load_manifest validates RunManifest JSON; BoundedSweepManifest Pydantic validators check seed pairs and n_cells; tests/unit/attacks/test_run_logger.py and test_bounded_sweep_manifest.py cover some round trips.
...
- Execution-time verification required: inspect the module responsible for manifest post read and historical compatibility validation; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Unvalidated old or malformed manifests can be accepted as current CP2 evidence. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for manifest post read and historical compatibility validation. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for manifest post read and historical compatibility validation if the decision gate authorizes changes.
...
- Production changes unrelated to manifest post read and historical compatibility validation.
...
- Intended canonical owner: the narrow package or module responsible for manifest post read and historical compatibility validation, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: manifest post read and historical compatibility validation. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify manifest post read and historical compatibility validation without changing unrelated behavior.
- architecture/import tests: verify manifest post read and historical compatibility validation without changing unrelated behavior.
- lifecycle tests: verify manifest post read and historical compatibility validation without changing unrelated behavior.
- resume tests: verify manifest post read and historical compatibility validation without changing unrelated behavior.
- corruption tests: verify manifest post read and historical compatibility validation without changing unrelated behavior.
- manifest round-trip tests: verify manifest post read and historical compatibility validation without changing unrelated behavior.
- historical fixture tests: verify manifest post read and historical compatibility validation without changing unrelated behavior.
- seed and pairing tests: verify manifest post read and historical compatibility validation without changing unrelated behavior.
- B4 determinism tests: verify manifest post read and historical compatibility validation without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for manifest post read and historical compatibility validation.
...
- Update only terminology directly tied to manifest post read and historical compatibility validation.
...
- Prerequisites: TKT-034, TKT-037, TKT-033.
- Successors: TKT-065.
- Sibling tickets: TKT-031, TKT-034, TKT-037, TKT-030.
- Potential overlap risks: work under TKT-006 may touch nearby surfaces; use the parent epic child map before editing.
