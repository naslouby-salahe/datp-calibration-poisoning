# TKT-025 — Lifecycle Marker State Classification
...
- Blocks: TKT-029, TKT-033, TKT-065
...
Define one lifecycle state interpreter for absent, in-progress, done, aborted, and corrupt markers. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-005. Its primary ownership boundary is narrower than the parent epic: lifecycle marker state classification.

## Problem Statement

Lifecycle classification can be fixed and tested without changing CP2 paths.
...
- Source location: src/datp/artifacts/lifecycle.py check_run_state returns CORRUPT unless exactly one marker exists, then DONE/ABORTED/IN_PROGRESS; tests/unit/artifacts/test_artifacts.py covers marker combinations.
...
- Execution-time verification required: inspect the module responsible for lifecycle marker state classification; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Marker disagreement can make status/resume unsafe. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for lifecycle marker state classification. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for lifecycle marker state classification if the decision gate authorizes changes.
...
- Production changes unrelated to lifecycle marker state classification.
...
- Intended canonical owner: the narrow package or module responsible for lifecycle marker state classification, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: lifecycle marker state classification. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify lifecycle marker state classification without changing unrelated behavior.
- architecture/import tests: verify lifecycle marker state classification without changing unrelated behavior.
- CLI tests: verify lifecycle marker state classification without changing unrelated behavior.
- artifact path tests: verify lifecycle marker state classification without changing unrelated behavior.
- lifecycle tests: verify lifecycle marker state classification without changing unrelated behavior.
- resume tests: verify lifecycle marker state classification without changing unrelated behavior.
- corruption tests: verify lifecycle marker state classification without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for lifecycle marker state classification.
...
- Update only terminology directly tied to lifecycle marker state classification.
...
- Prerequisites: TKT-004.
- Successors: TKT-029, TKT-033, TKT-065.
- Sibling tickets: TKT-027, TKT-029, TKT-033, TKT-036.
