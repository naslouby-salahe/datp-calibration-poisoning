# TKT-040 — Malformed Score Artifact Handling
...
Define exact failure behavior for missing, empty, malformed, and schema-invalid score artifacts. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-007. Its primary ownership boundary is narrower than the parent epic: malformed score artifact handling.

## Problem Statement

Malformed input handling has distinct tests and rollback from normal adapter behavior.
...
- Source location: src/datp/scoring/loading.py raises FileNotFoundError for missing dirs/files and validate_score_artifact handles schema validation; integration scoring tests cover missing manifest and expected column.
...
- Execution-time verification required: inspect the module responsible for malformed score artifact handling; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Weak malformed handling can accept wrong score inputs or hide corrupted artifacts. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for malformed score artifact handling. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for malformed score artifact handling if the decision gate authorizes changes.
...
- Production changes unrelated to malformed score artifact handling.
...
- Intended canonical owner: the narrow package or module responsible for malformed score artifact handling, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: malformed score artifact handling. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify malformed score artifact handling without changing unrelated behavior.
- architecture/import tests: verify malformed score artifact handling without changing unrelated behavior.
- artifact path tests: verify malformed score artifact handling without changing unrelated behavior.
- manifest round-trip tests: verify malformed score artifact handling without changing unrelated behavior.
- historical fixture tests: verify malformed score artifact handling without changing unrelated behavior.
- score-stage tests: verify malformed score artifact handling without changing unrelated behavior.
- malformed input tests: verify malformed score artifact handling without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for malformed score artifact handling.
...
- Update only terminology directly tied to malformed score artifact handling.
...
- Prerequisites: TKT-028, TKT-035.
- Successors: TKT-062.
- Sibling tickets: TKT-026, TKT-028, TKT-032, TKT-035, TKT-038.
- Potential overlap risks: work under TKT-007 may touch nearby surfaces; use the parent epic child map before editing.
