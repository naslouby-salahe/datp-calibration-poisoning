# TKT-029 — Artifact Completeness and Corruption Detection
...
- Priority: P1
- Dependencies: TKT-027, TKT-025
- Blocks: TKT-033, TKT-039, TKT-065
...
Define completeness checks for CP2 run and aggregate artifacts, including corrupt partial outputs. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-005. Its primary ownership boundary is narrower than the parent epic: artifact completeness and corruption detection.

## Problem Statement

Completeness validation has a distinct rollback boundary from marker classification.
...
- Source location: src/datp/artifacts/existence.py validates generic metrics.json and treats JSON/schema errors as missing; CP2 aggregate manifest writes exist in src/datp/attacks/bounded_sweep_run.py; CP2 per-run paths include cell_metrics, seed_record, provenance.
...
- Execution-time verification required: inspect the module responsible for artifact completeness and corruption detection; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Incomplete outputs can be accepted as valid evidence or rerun unnecessarily. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for artifact completeness and corruption detection. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for artifact completeness and corruption detection if the decision gate authorizes changes.
...
- Production changes unrelated to artifact completeness and corruption detection.
...
- Intended canonical owner: the narrow package or module responsible for artifact completeness and corruption detection, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: artifact completeness and corruption detection. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify artifact completeness and corruption detection without changing unrelated behavior.
- architecture/import tests: verify artifact completeness and corruption detection without changing unrelated behavior.
- artifact path tests: verify artifact completeness and corruption detection without changing unrelated behavior.
- lifecycle tests: verify artifact completeness and corruption detection without changing unrelated behavior.
- resume tests: verify artifact completeness and corruption detection without changing unrelated behavior.
- corruption tests: verify artifact completeness and corruption detection without changing unrelated behavior.
- manifest round-trip tests: verify artifact completeness and corruption detection without changing unrelated behavior.
- historical fixture tests: verify artifact completeness and corruption detection without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for artifact completeness and corruption detection.
...
- Update only terminology directly tied to artifact completeness and corruption detection.
...
- Prerequisites: TKT-027, TKT-025.
- Successors: TKT-033, TKT-039, TKT-065.
- Sibling tickets: TKT-027, TKT-025, TKT-033, TKT-036.
- Potential overlap risks: work under TKT-005 may touch nearby surfaces; use the parent epic child map before editing.
