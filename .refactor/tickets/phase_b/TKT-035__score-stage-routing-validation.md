# TKT-035 — Score Stage Routing Validation
...
Validate calibration, benign-test, and attack-test stage routing before CP2 adaptation. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-007. Its primary ownership boundary is narrower than the parent epic: score stage routing validation.

## Problem Statement

Stage routing failures are independently testable from reservoir enforcement.
...
- Source location: src/datp/core.enums ScoringStage is used by src/datp/scoring/loading.py and src/datp/attacks/real_score_loader.py; real_score_loader loads CAL via cal_loading and test stages from score_dir.
...
- Execution-time verification required: inspect the module responsible for score stage routing validation; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Stage mixing can turn calibration attack into test-data contamination. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for score stage routing validation. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for score stage routing validation if the decision gate authorizes changes.
...
- Production changes unrelated to score stage routing validation.
...
- Intended canonical owner: the narrow package or module responsible for score stage routing validation, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: score stage routing validation. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify score stage routing validation without changing unrelated behavior.
- architecture/import tests: verify score stage routing validation without changing unrelated behavior.
- artifact path tests: verify score stage routing validation without changing unrelated behavior.
- manifest round-trip tests: verify score stage routing validation without changing unrelated behavior.
- historical fixture tests: verify score stage routing validation without changing unrelated behavior.
- score-stage tests: verify score stage routing validation without changing unrelated behavior.
- malformed input tests: verify score stage routing validation without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for score stage routing validation.
...
- Update only terminology directly tied to score stage routing validation.
...
- Prerequisites: TKT-028, TKT-006.
- Successors: TKT-062.
- Sibling tickets: TKT-026, TKT-028, TKT-032, TKT-038, TKT-040.
