# TKT-030 — Generic Atomic JSON and CSV Transport Delegation
...
Delegate transport mechanics to generic artifact I/O where schema ownership remains CP2-specific. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-006. Its primary ownership boundary is narrower than the parent epic: generic atomic json and csv transport delegation.

## Problem Statement

Transport mechanics are independently testable from schema construction.
...
- Source location: src/datp/artifacts/io.py provides write_json_atomic and write_csv; src/datp/attacks/run_logger.py emit_manifest uses path.write_text; src/datp/attacks/bounded_sweep_run.py write_nbaiot_bounded_sweep_manifest uses write_text.
...
- Execution-time verification required: inspect the module responsible for generic atomic json and csv transport delegation; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Non-atomic writes can leave partial manifests that appear resumable. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for generic atomic json and csv transport delegation. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for generic atomic json and csv transport delegation if the decision gate authorizes changes.
...
- Production changes unrelated to generic atomic json and csv transport delegation.
...
- SER-CSV: CSV output fields and order-sensitive report consumers remain compatible. Status: preserved unchanged unless an explicitly additive compatibility path is required.
...
- Intended canonical owner: the narrow package or module responsible for generic atomic json and csv transport delegation, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: generic atomic json and csv transport delegation. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- CSV: preserve columns and report consumers.
...
- Reporting: preserve output names and sidecar schemas.
...
- unit tests: verify generic atomic json and csv transport delegation without changing unrelated behavior.
- architecture/import tests: verify generic atomic json and csv transport delegation without changing unrelated behavior.
- lifecycle tests: verify generic atomic json and csv transport delegation without changing unrelated behavior.
- resume tests: verify generic atomic json and csv transport delegation without changing unrelated behavior.
- corruption tests: verify generic atomic json and csv transport delegation without changing unrelated behavior.
- manifest round-trip tests: verify generic atomic json and csv transport delegation without changing unrelated behavior.
- historical fixture tests: verify generic atomic json and csv transport delegation without changing unrelated behavior.
- report compatibility tests: verify generic atomic json and csv transport delegation without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for generic atomic json and csv transport delegation.
...
- Update only terminology directly tied to generic atomic json and csv transport delegation.
...
- Prerequisites: TKT-025.
- Successors: TKT-034, TKT-037.
- Sibling tickets: TKT-031, TKT-034, TKT-037, TKT-039.
