# TKT-032 — CP2 Runtime Score Collection Adapter
...
Keep CP2 ClientScores and ScoreCollection as runtime adapters over validated generic scores. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-007. Its primary ownership boundary is narrower than the parent epic: cp2 runtime score collection adapter.

## Problem Statement

Runtime containers encode eligibility/victim semantics, distinct from persisted schema.
...
- Source location: src/datp/attacks/score_containers.py ClientScores stores cal, test_benign, test_attack; ScoreCollection computes eligible/pending ids with identify_eligible; src/datp/attacks/real_score_loader.py builds ScoreCollection from generic loaders.
...
- Execution-time verification required: inspect the module responsible for cp2 runtime score collection adapter; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Merging schema and runtime containers can leak attack semantics into generic scoring. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for cp2 runtime score collection adapter. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for cp2 runtime score collection adapter if the decision gate authorizes changes.
...
- Production changes unrelated to cp2 runtime score collection adapter.
...
- Intended canonical owner: the narrow package or module responsible for cp2 runtime score collection adapter, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: cp2 runtime score collection adapter. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify cp2 runtime score collection adapter without changing unrelated behavior.
- architecture/import tests: verify cp2 runtime score collection adapter without changing unrelated behavior.
- score-stage tests: verify cp2 runtime score collection adapter without changing unrelated behavior.
- malformed input tests: verify cp2 runtime score collection adapter without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for cp2 runtime score collection adapter.
...
- Update only terminology directly tied to cp2 runtime score collection adapter.
...
- Successors: TKT-053.
- Sibling tickets: TKT-026, TKT-028, TKT-035, TKT-038, TKT-040.
