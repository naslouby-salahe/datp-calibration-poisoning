# TKT-031 — Runtime Result to Persisted Row Conversion Owner
...
Make the runtime-to-BoundedSweepResultRow conversion a single explicit owner. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-006. Its primary ownership boundary is narrower than the parent epic: runtime result to persisted row conversion owner.

## Problem Statement

Row conversion is separable from per-run manifest and aggregate manifest construction.
...
- Source location: src/datp/attacks/bounded_sweep_run.py _row_for_cell builds BoundedSweepResultRow from SweepCellSpec, SweepCellResult, metrics, blast/spillover, and SeedRecordModel.
...
- Execution-time verification required: inspect the module responsible for runtime result to persisted row conversion owner; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Multiple row assemblers can omit fields or compute identity differently. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for runtime result to persisted row conversion owner. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for runtime result to persisted row conversion owner if the decision gate authorizes changes.
...
- Production changes unrelated to runtime result to persisted row conversion owner.
...
- Intended canonical owner: the narrow package or module responsible for runtime result to persisted row conversion owner, selected only after verification.
...
- unit tests: verify runtime result to persisted row conversion owner without changing unrelated behavior.
- architecture/import tests: verify runtime result to persisted row conversion owner without changing unrelated behavior.
- manifest round-trip tests: verify runtime result to persisted row conversion owner without changing unrelated behavior.
- historical fixture tests: verify runtime result to persisted row conversion owner without changing unrelated behavior.
- seed and pairing tests: verify runtime result to persisted row conversion owner without changing unrelated behavior.
- B4 determinism tests: verify runtime result to persisted row conversion owner without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for runtime result to persisted row conversion owner.
...
- Update only terminology directly tied to runtime result to persisted row conversion owner.
...
- Prerequisites: TKT-023.
- Successors: TKT-037.
- Sibling tickets: TKT-034, TKT-037, TKT-030, TKT-039.
