# TKT-034 — Per Run Manifest Construction Boundary
...
Centralize RunManifest construction and emission while preserving field names. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-006. Its primary ownership boundary is narrower than the parent epic: per run manifest construction boundary.

## Problem Statement

Per-run manifest behavior is different from aggregate bounded manifest behavior.
...
- Source location: src/datp/attacks/run_logger.py build_manifest constructs RunManifest and emit_manifest writes run_manifest.json; src/datp/attacks/run_manifest.py defines schema and provenance.
...
- Execution-time verification required: inspect the module responsible for per run manifest construction boundary; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Manifest construction drift loses provenance or emits unlocked mu_flag values. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for per run manifest construction boundary. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for per run manifest construction boundary if the decision gate authorizes changes.
...
- Production changes unrelated to per run manifest construction boundary.
...
- Intended canonical owner: the narrow package or module responsible for per run manifest construction boundary, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: per run manifest construction boundary. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify per run manifest construction boundary without changing unrelated behavior.
- architecture/import tests: verify per run manifest construction boundary without changing unrelated behavior.
- artifact path tests: verify per run manifest construction boundary without changing unrelated behavior.
- manifest round-trip tests: verify per run manifest construction boundary without changing unrelated behavior.
- historical fixture tests: verify per run manifest construction boundary without changing unrelated behavior.
- seed and pairing tests: verify per run manifest construction boundary without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for per run manifest construction boundary.
...
- Update only terminology directly tied to per run manifest construction boundary.
...
- Prerequisites: TKT-023, TKT-027.
- Successors: TKT-039.
- Sibling tickets: TKT-031, TKT-037, TKT-030, TKT-039.
