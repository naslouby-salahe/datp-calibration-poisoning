# TKT-044 — Score Loading versus Artifact Layout Boundary
...
Clarify whether scoring may depend on artifact layout primitives or should receive resolved paths. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-008. Its primary ownership boundary is narrower than the parent epic: score loading versus artifact layout boundary.

## Problem Statement

This boundary is independent from metric validation and public re-export work.
...
- Source location: src/datp/scoring/cal_loading.py imports ArtifactLayout and TrainingCellId; src/datp/scoring/loading.py imports PathToken; src/datp/attacks/real_score_loader.py also resolves ArtifactLayout.
...
- Execution-time verification required: inspect the module responsible for score loading versus artifact layout boundary; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Improper boundary can lock scoring to artifact filesystem assumptions. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for score loading versus artifact layout boundary. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for score loading versus artifact layout boundary if the decision gate authorizes changes.
...
- Production changes unrelated to score loading versus artifact layout boundary.
...
- Intended canonical owner: the narrow package or module responsible for score loading versus artifact layout boundary, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: score loading versus artifact layout boundary. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify score loading versus artifact layout boundary without changing unrelated behavior.
- architecture/import tests: verify score loading versus artifact layout boundary without changing unrelated behavior.
- artifact path tests: verify score loading versus artifact layout boundary without changing unrelated behavior.
- score-stage tests: verify score loading versus artifact layout boundary without changing unrelated behavior.
- malformed input tests: verify score loading versus artifact layout boundary without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for score loading versus artifact layout boundary.
...
- Update only terminology directly tied to score loading versus artifact layout boundary.
...
- Prerequisites: TKT-041, TKT-028.
- Successors: TKT-062.
- Sibling tickets: TKT-041, TKT-043, TKT-052, TKT-045.
