# TKT-052 — Generic Metric Validation versus CP2 Metric Validation Taxonomy
...
Define which package validates generic metrics, CP2 paired metrics, provenance, and report inputs. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-008. Its primary ownership boundary is narrower than the parent epic: generic metric validation versus cp2 metric validation taxonomy.

## Problem Statement

Validation taxonomy can be accepted independently from import movement.
...
- Source location: src/datp/evaluation/artifact_validation.py validates generic metrics payloads; src/datp/validation/provenance_gate.py imports attacks.run_manifest; src/datp/reporting/build.py validates report metrics via validate_metrics_payload and recomputation.
...
- Execution-time verification required: inspect the module responsible for generic metric validation versus cp2 metric validation taxonomy; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Conflicting validation can reject good artifacts or accept invalid CP2 results. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for generic metric validation versus cp2 metric validation taxonomy. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for generic metric validation versus cp2 metric validation taxonomy if the decision gate authorizes changes.
...
- Production changes unrelated to generic metric validation versus cp2 metric validation taxonomy.
...
- Intended canonical owner: the narrow package or module responsible for generic metric validation versus cp2 metric validation taxonomy, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: generic metric validation versus cp2 metric validation taxonomy. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify generic metric validation versus cp2 metric validation taxonomy without changing unrelated behavior.
- architecture/import tests: verify generic metric validation versus cp2 metric validation taxonomy without changing unrelated behavior.
- manifest round-trip tests: verify generic metric validation versus cp2 metric validation taxonomy without changing unrelated behavior.
- historical fixture tests: verify generic metric validation versus cp2 metric validation taxonomy without changing unrelated behavior.
- policy semantic tests: verify generic metric validation versus cp2 metric validation taxonomy without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for generic metric validation versus cp2 metric validation taxonomy.
...
- Update only terminology directly tied to generic metric validation versus cp2 metric validation taxonomy.
...
- Prerequisites: TKT-041, TKT-053.
- Successors: TKT-062.
- Sibling tickets: TKT-041, TKT-043, TKT-044, TKT-045.
