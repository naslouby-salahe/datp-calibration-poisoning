# TKT-046 — Generic Rendering Input Model Boundary
...
- Priority: P1
- Dependencies: TKT-042
- Blocks: TKT-054, TKT-059
...
Ensure generic reporting rendering receives prepared data, not raw CP2 scientific objects. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-009. Its primary ownership boundary is narrower than the parent epic: generic rendering input model boundary.

## Problem Statement

Rendering input models can be validated without changing CLI routing.
...
- Source location: src/datp/reporting/engine.py renders templates; reporting/figures.py and tables.py generate output; reporting/build.py currently prepares data and invokes rendering.
...
- Execution-time verification required: inspect the module responsible for generic rendering input model boundary; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Generic rendering can become coupled to CP2 metrics or B4-only fields. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for generic rendering input model boundary. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for generic rendering input model boundary if the decision gate authorizes changes.
...
- Production changes unrelated to generic rendering input model boundary.
...
- Intended canonical owner: the narrow package or module responsible for generic rendering input model boundary, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: generic rendering input model boundary. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify generic rendering input model boundary without changing unrelated behavior.
- architecture/import tests: verify generic rendering input model boundary without changing unrelated behavior.
- manifest round-trip tests: verify generic rendering input model boundary without changing unrelated behavior.
- historical fixture tests: verify generic rendering input model boundary without changing unrelated behavior.
- report compatibility tests: verify generic rendering input model boundary without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for generic rendering input model boundary.
...
- Update only terminology directly tied to generic rendering input model boundary.
...
- Prerequisites: TKT-042.
- Successors: TKT-054, TKT-059.
- Sibling tickets: TKT-042, TKT-047, TKT-054, TKT-059.
