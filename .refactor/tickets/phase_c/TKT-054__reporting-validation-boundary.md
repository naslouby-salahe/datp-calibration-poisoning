# TKT-054 — Reporting Validation Boundary
...
Keep report input validation separate from generic metric validation and CP2 protocol validation. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-009. Its primary ownership boundary is narrower than the parent epic: reporting validation boundary.

## Problem Statement

Reporting validation has a separate acceptance surface from rendering output names.
...
- Source location: src/datp/reporting/validation.py validates main-body baseline role; reporting/build.py validates payloads and recomputes metrics; datp.validation owns broader audit/provenance checks.
...
- Execution-time verification required: inspect the module responsible for reporting validation boundary; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Validation duplication can produce inconsistent report acceptance. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for reporting validation boundary. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for reporting validation boundary if the decision gate authorizes changes.
...
- Production changes unrelated to reporting validation boundary.
...
- Intended canonical owner: the narrow package or module responsible for reporting validation boundary, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: reporting validation boundary. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify reporting validation boundary without changing unrelated behavior.
- architecture/import tests: verify reporting validation boundary without changing unrelated behavior.
- manifest round-trip tests: verify reporting validation boundary without changing unrelated behavior.
- historical fixture tests: verify reporting validation boundary without changing unrelated behavior.
- report compatibility tests: verify reporting validation boundary without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for reporting validation boundary.
...
- Update only terminology directly tied to reporting validation boundary.
...
- Prerequisites: TKT-046, TKT-052.
- Successors: TKT-062.
- Sibling tickets: TKT-042, TKT-047, TKT-046, TKT-059.
