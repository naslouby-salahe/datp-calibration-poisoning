# TKT-045 — Public Re Export and Architecture Test Preservation
...
Preserve public imports while hardening architecture tests around semantic boundaries. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-008. Its primary ownership boundary is narrower than the parent epic: public re export and architecture test preservation.

## Problem Statement

Public import compatibility must be tested independently from internal refactors.
...
- Source location: src/datp/evaluation/__init__.py exposes lazy public attributes; src/datp/config/__init__.py and statistics/__init__.py re-export public symbols; tests/unit/core/test_model_boundary_architecture.py and test_no_scientific_policy_leaks.py exist.
...
- Execution-time verification required: inspect the module responsible for public re export and architecture test preservation; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Import cleanup can break public consumers even if tests pass internally. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for public re export and architecture test preservation. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for public re export and architecture test preservation if the decision gate authorizes changes.
...
- Production changes unrelated to public re export and architecture test preservation.
...
- Intended canonical owner: the narrow package or module responsible for public re export and architecture test preservation, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: public re export and architecture test preservation. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify public re export and architecture test preservation without changing unrelated behavior.
- architecture/import tests: verify public re export and architecture test preservation without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for public re export and architecture test preservation.
...
- Update only terminology directly tied to public re export and architecture test preservation.
...
- Prerequisites: TKT-041.
- Successors: TKT-068.
- Sibling tickets: TKT-041, TKT-043, TKT-044, TKT-052.
