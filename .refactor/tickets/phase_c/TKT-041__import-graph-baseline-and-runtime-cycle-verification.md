# TKT-041 — Import Graph Baseline and Runtime Cycle Verification
...
- Priority: P1
- Dependencies: TKT-028
- Blocks: TKT-043, TKT-044, TKT-045
...
Capture actual package dependency graph and distinguish package-level cycles from runtime circular imports. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-008. Its primary ownership boundary is narrower than the parent epic: import graph baseline and runtime cycle verification.

## Problem Statement

The graph baseline is prerequisite evidence, not the same fix as boundary refactors.
...
- Source location: rg confirms artifacts.existence imports evaluation.artifact_validation, evaluation.metrics imports scoring.loading, scoring.loading/cal_loading import artifacts names/layout; evaluation.__init__ uses lazy __getattr__ to avoid import cycles.
...
- Execution-time verification required: inspect the module responsible for import graph baseline and runtime cycle verification; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Fixing package direction without graph evidence can remove needed validation or create new cycles. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for import graph baseline and runtime cycle verification. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for import graph baseline and runtime cycle verification if the decision gate authorizes changes.
...
- Production changes unrelated to import graph baseline and runtime cycle verification.
...
- Intended canonical owner: the narrow package or module responsible for import graph baseline and runtime cycle verification, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: import graph baseline and runtime cycle verification. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify import graph baseline and runtime cycle verification without changing unrelated behavior.
- architecture/import tests: verify import graph baseline and runtime cycle verification without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for import graph baseline and runtime cycle verification.
...
- Update only terminology directly tied to import graph baseline and runtime cycle verification.
...
- Prerequisites: TKT-028.
- Successors: TKT-043, TKT-044, TKT-045.
- Sibling tickets: TKT-043, TKT-044, TKT-052, TKT-045.
