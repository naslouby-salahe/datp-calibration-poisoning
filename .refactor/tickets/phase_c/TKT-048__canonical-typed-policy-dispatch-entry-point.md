# TKT-048 — Canonical Typed Policy Dispatch Entry Point
...
- Priority: P0
- Dependencies: TKT-011, TKT-019
- Blocks: TKT-050, TKT-049, TKT-051, TKT-063
...
Confirm or establish one CP2 policy dispatcher that routes typed policies to separate implementations. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-010. Its primary ownership boundary is narrower than the parent epic: canonical typed policy dispatch entry point.

## Problem Statement

Dispatcher ownership is independent from B3 rejection and metric ownership.
...
- Source location: src/datp/attacks/cell_runner.py recompute_pair dispatches ThresholdPolicy.B1_GLOBAL, B2_PERSONALIZED, B4_CLUSTER and assert_never otherwise.
...
- Execution-time verification required: inspect the module responsible for canonical typed policy dispatch entry point; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Multiple dispatchers can apply inconsistent B3, B4, or default behavior. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for canonical typed policy dispatch entry point. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for canonical typed policy dispatch entry point if the decision gate authorizes changes.
...
- Production changes unrelated to canonical typed policy dispatch entry point.
...
- Intended canonical owner: the narrow package or module responsible for canonical typed policy dispatch entry point, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: canonical typed policy dispatch entry point. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify canonical typed policy dispatch entry point without changing unrelated behavior.
- architecture/import tests: verify canonical typed policy dispatch entry point without changing unrelated behavior.
- B4 determinism tests: verify canonical typed policy dispatch entry point without changing unrelated behavior.
- policy semantic tests: verify canonical typed policy dispatch entry point without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for canonical typed policy dispatch entry point.
...
- Update only terminology directly tied to canonical typed policy dispatch entry point.
...
- Prerequisites: TKT-011, TKT-019.
- Successors: TKT-050, TKT-049, TKT-051, TKT-063.
- Sibling tickets: TKT-050, TKT-049, TKT-051, TKT-053, TKT-055.
