# TKT-059 — Report Output Compatibility and CLI Routing
...
Preserve report file names, sidecars, table/figure outputs, and report CLI routes after boundary cleanup. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-009. Its primary ownership boundary is narrower than the parent epic: report output compatibility and cli routing.

## Problem Statement

Output compatibility and CLI routing are user-facing surfaces independent from internal import classification.
...
- Source location: src/datp/app/cli/report.py registers stats, validate, figures, tables, all commands; reporting/build.py writes JSON, CSV, figures, tables, and sidecars via ArtifactFile/ArtifactDir.
...
- Execution-time verification required: inspect the module responsible for report output compatibility and cli routing; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Report consumers can break if file names, routes, or sidecar schemas drift. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for report output compatibility and cli routing. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for report output compatibility and cli routing if the decision gate authorizes changes.
...
- Production changes unrelated to report output compatibility and cli routing.
...
- Intended canonical owner: the narrow package or module responsible for report output compatibility and cli routing, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: report output compatibility and cli routing. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify report output compatibility and cli routing without changing unrelated behavior.
- architecture/import tests: verify report output compatibility and cli routing without changing unrelated behavior.
- CLI tests: verify report output compatibility and cli routing without changing unrelated behavior.
- report compatibility tests: verify report output compatibility and cli routing without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for report output compatibility and cli routing.
...
- Update only terminology directly tied to report output compatibility and cli routing.
...
- Prerequisites: TKT-046, TKT-054.
- Successors: TKT-062.
- Sibling tickets: TKT-042, TKT-047, TKT-046, TKT-054.
- Potential overlap risks: work under TKT-009 may touch nearby surfaces; use the parent epic child map before editing.
