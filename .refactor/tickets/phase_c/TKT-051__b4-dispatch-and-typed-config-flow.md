# TKT-051 — B4 Dispatch and Typed Config Flow
...
- Priority: P0
- Dependencies: TKT-048, TKT-019, TKT-024
- Blocks: TKT-053
...
Preserve B4 cluster/effective threshold/decomposition behavior behind dispatcher using typed config. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-010. Its primary ownership boundary is narrower than the parent epic: b4 dispatch and typed config flow.

## Problem Statement

B4 dispatch is separable from B1/B2 and metric serialization.
...
- Source location: src/datp/attacks/b4_recompute.py compute_b4_pair runs clean B4, frozen clean assignment aggregation, full poisoned B4, and churn residual; cell_runner recompute_pair routes B4.
...
- Execution-time verification required: inspect the module responsible for b4 dispatch and typed config flow; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

B4 dispatch drift can hide decomposition or change effective thresholds. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for b4 dispatch and typed config flow. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for b4 dispatch and typed config flow if the decision gate authorizes changes.
...
- Production changes unrelated to b4 dispatch and typed config flow.
...
- Intended canonical owner: the narrow package or module responsible for b4 dispatch and typed config flow, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: b4 dispatch and typed config flow. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify b4 dispatch and typed config flow without changing unrelated behavior.
- architecture/import tests: verify b4 dispatch and typed config flow without changing unrelated behavior.
- B4 determinism tests: verify b4 dispatch and typed config flow without changing unrelated behavior.
- policy semantic tests: verify b4 dispatch and typed config flow without changing unrelated behavior.
- score-stage tests: verify b4 dispatch and typed config flow without changing unrelated behavior.
- malformed input tests: verify b4 dispatch and typed config flow without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for b4 dispatch and typed config flow.
...
- Update only terminology directly tied to b4 dispatch and typed config flow.
...
- Prerequisites: TKT-048, TKT-019, TKT-024.
- Successors: TKT-053.
- Sibling tickets: TKT-048, TKT-050, TKT-049, TKT-053, TKT-055.
