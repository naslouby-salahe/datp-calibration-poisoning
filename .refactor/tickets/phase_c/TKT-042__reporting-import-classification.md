# TKT-042 — Reporting Import Classification
...
Classify current reporting imports and verify whether reporting imports CP2 attack modules. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-009. Its primary ownership boundary is narrower than the parent epic: reporting import classification.

## Problem Statement

Import classification can close no-code if existing boundaries are already correct.
...
- Source location: Source read of reporting/build.py, engine.py, validation.py showed no datp.attacks imports in inspected sections; reporting/build.py imports scoring, artifacts, evaluation, statistics, validation, and config.
...
- Execution-time verification required: inspect the module responsible for reporting import classification; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

A false assumption here can create unnecessary adapter work or leave CP2 coupling hidden. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for reporting import classification. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for reporting import classification if the decision gate authorizes changes.
...
- Production changes unrelated to reporting import classification.
...
- Intended canonical owner: the narrow package or module responsible for reporting import classification, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: reporting import classification. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify reporting import classification without changing unrelated behavior.
- architecture/import tests: verify reporting import classification without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for reporting import classification.
...
- Update only terminology directly tied to reporting import classification.
...
- Successors: TKT-047, TKT-046.
- Sibling tickets: TKT-047, TKT-046, TKT-054, TKT-059.
