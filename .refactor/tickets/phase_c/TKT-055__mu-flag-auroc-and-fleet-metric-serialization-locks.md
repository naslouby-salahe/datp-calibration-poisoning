# TKT-055 — Mu Flag AUROC and Fleet Metric Serialization Locks
...
- Priority: P0
- Dependencies: TKT-053, TKT-006
- Blocks: TKT-031, TKT-065
...
Lock mu_flag threshold ownership, AUROC invariance, fleet metrics, and serialization mapping. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-010. Its primary ownership boundary is narrower than the parent epic: mu flag auroc and fleet metric serialization locks.

## Problem Statement

These guard metrics share paired-result semantics and are independently auditable.
...
- Source location: src/datp/attacks/bounded_sweep_cell.py lock_mu_flag_threshold computes from clean B1; src/datp/attacks/metric_engine.py compute_mu_flag_threshold owns math and compute_auroc_records uses test scores; BoundedSweepResultRow stores cv_fpr, mean_fpr, coverage, mu_flag_triggered, auroc_invariant.
...
- Execution-time verification required: inspect the module responsible for mu flag auroc and fleet metric serialization locks; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Incorrect guard metric serialization changes kill-trigger and safety interpretation. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for mu flag auroc and fleet metric serialization locks. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for mu flag auroc and fleet metric serialization locks if the decision gate authorizes changes.
...
- Production changes unrelated to mu flag auroc and fleet metric serialization locks.
...
- Intended canonical owner: the narrow package or module responsible for mu flag auroc and fleet metric serialization locks, selected only after verification.
...
- unit tests: verify mu flag auroc and fleet metric serialization locks without changing unrelated behavior.
- architecture/import tests: verify mu flag auroc and fleet metric serialization locks without changing unrelated behavior.
- manifest round-trip tests: verify mu flag auroc and fleet metric serialization locks without changing unrelated behavior.
- historical fixture tests: verify mu flag auroc and fleet metric serialization locks without changing unrelated behavior.
- seed and pairing tests: verify mu flag auroc and fleet metric serialization locks without changing unrelated behavior.
- score-stage tests: verify mu flag auroc and fleet metric serialization locks without changing unrelated behavior.
- malformed input tests: verify mu flag auroc and fleet metric serialization locks without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for mu flag auroc and fleet metric serialization locks.
...
- Update only terminology directly tied to mu flag auroc and fleet metric serialization locks.
...
- Prerequisites: TKT-053, TKT-006.
- Successors: TKT-031, TKT-065.
- Sibling tickets: TKT-048, TKT-050, TKT-049, TKT-051, TKT-053.
