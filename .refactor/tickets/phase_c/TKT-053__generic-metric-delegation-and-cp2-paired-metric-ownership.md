# TKT-053 — Generic Metric Delegation and CP2 Paired Metric Ownership
...
- Priority: P0
- Dependencies: TKT-049, TKT-051
- Blocks: TKT-055, TKT-062
...
Separate generic binary metric formulas from CP2 paired attack metrics. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-010. Its primary ownership boundary is narrower than the parent epic: generic metric delegation and cp2 paired metric ownership.

## Problem Statement

Metric ownership is independently testable from policy dispatch.
...
- Source location: src/datp/evaluation.metrics owns generic evaluation; src/datp/attacks/metric_engine.py owns DeltaTauEntry, FleetFprMetrics, AurocRecord, MetricResult and computes CP2 metrics.
...
- Execution-time verification required: inspect the module responsible for generic metric delegation and cp2 paired metric ownership; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Formula duplication can produce inconsistent reported values. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for generic metric delegation and cp2 paired metric ownership. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for generic metric delegation and cp2 paired metric ownership if the decision gate authorizes changes.
...
- Production changes unrelated to generic metric delegation and cp2 paired metric ownership.
...
- Intended canonical owner: the narrow package or module responsible for generic metric delegation and cp2 paired metric ownership, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: generic metric delegation and cp2 paired metric ownership. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify generic metric delegation and cp2 paired metric ownership without changing unrelated behavior.
- architecture/import tests: verify generic metric delegation and cp2 paired metric ownership without changing unrelated behavior.
- manifest round-trip tests: verify generic metric delegation and cp2 paired metric ownership without changing unrelated behavior.
- historical fixture tests: verify generic metric delegation and cp2 paired metric ownership without changing unrelated behavior.
- policy semantic tests: verify generic metric delegation and cp2 paired metric ownership without changing unrelated behavior.
- score-stage tests: verify generic metric delegation and cp2 paired metric ownership without changing unrelated behavior.
- malformed input tests: verify generic metric delegation and cp2 paired metric ownership without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for generic metric delegation and cp2 paired metric ownership.
...
- Update only terminology directly tied to generic metric delegation and cp2 paired metric ownership.
...
- Prerequisites: TKT-049, TKT-051, TKT-052.
- Successors: TKT-055, TKT-062.
- Sibling tickets: TKT-048, TKT-050, TKT-049, TKT-051, TKT-055.
