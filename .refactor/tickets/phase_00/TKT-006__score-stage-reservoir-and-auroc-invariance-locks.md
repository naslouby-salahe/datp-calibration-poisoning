# TKT-006 — Score Stage Reservoir and AUROC Invariance Locks
...
- Blocks: TKT-035, TKT-038, TKT-055
...
Lock score-stage separation, reservoir safety, and AUROC test-score invariance. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-001. Its primary ownership boundary is narrower than the parent epic: score stage reservoir and auroc invariance locks.

## Problem Statement

Score-stage contamination is independently testable and can be rolled back without touching identity or lifecycle code.
...
- Source location: src/datp/scoring/schema.py owns SCORE_COLUMN; src/datp/scoring/loading.py loads ScoringStage paths; src/datp/attacks/reservoir.py builds reservoirs from clean calibration scores; src/datp/attacks/metric_engine.py computes AUROC from test_benign and test_attack.
...
- Execution-time verification required: inspect the module responsible for score stage reservoir and auroc invariance locks; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Reservoir or AUROC contamination silently changes CP2 from calibration poisoning into an out-of-scope attack. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for score stage reservoir and auroc invariance locks. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for score stage reservoir and auroc invariance locks if the decision gate authorizes changes.
...
- Production changes unrelated to score stage reservoir and auroc invariance locks.
...
- Intended canonical owner: the narrow package or module responsible for score stage reservoir and auroc invariance locks, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: score stage reservoir and auroc invariance locks. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify score stage reservoir and auroc invariance locks without changing unrelated behavior.
- architecture/import tests: verify score stage reservoir and auroc invariance locks without changing unrelated behavior.
- manifest round-trip tests: verify score stage reservoir and auroc invariance locks without changing unrelated behavior.
- historical fixture tests: verify score stage reservoir and auroc invariance locks without changing unrelated behavior.
- score-stage tests: verify score stage reservoir and auroc invariance locks without changing unrelated behavior.
- malformed input tests: verify score stage reservoir and auroc invariance locks without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for score stage reservoir and auroc invariance locks.
...
- Update only terminology directly tied to score stage reservoir and auroc invariance locks.
...
- Successors: TKT-035, TKT-038, TKT-055.
- Sibling tickets: TKT-001, TKT-002, TKT-003, TKT-004, TKT-005.
