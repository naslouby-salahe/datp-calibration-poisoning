# TKT-004 — Artifact Path and Lifecycle Compatibility Locks
...
- Blocks: TKT-027, TKT-025, TKT-029, TKT-033
...
Lock path strings, marker states, and corruption behavior independently from manifest schema tests. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-001. Its primary ownership boundary is narrower than the parent epic: artifact path and lifecycle compatibility locks.

## Problem Statement

Artifact compatibility can fail while scientific computations remain correct; it needs an independent safety net.
...
- Source location: src/datp/artifacts/poison_layout.py formats f_{fraction:.2f}, scope_{scope.value}, train_{seed}, poison_{seed}; src/datp/artifacts/lifecycle.py maps IN_PROGRESS, DONE, ABORTED, and conflicting markers to RunState; tests/unit/artifacts/test_artifacts.py covers marker states.
...
- Execution-time verification required: inspect the module responsible for artifact path and lifecycle compatibility locks; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Path or lifecycle drift can make outputs undiscoverable or make resume accept stale/partial results. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for artifact path and lifecycle compatibility locks. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for artifact path and lifecycle compatibility locks if the decision gate authorizes changes.
...
- Production changes unrelated to artifact path and lifecycle compatibility locks.
...
- Intended canonical owner: the narrow package or module responsible for artifact path and lifecycle compatibility locks, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: artifact path and lifecycle compatibility locks. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify artifact path and lifecycle compatibility locks without changing unrelated behavior.
- architecture/import tests: verify artifact path and lifecycle compatibility locks without changing unrelated behavior.
- CLI tests: verify artifact path and lifecycle compatibility locks without changing unrelated behavior.
- artifact path tests: verify artifact path and lifecycle compatibility locks without changing unrelated behavior.
- lifecycle tests: verify artifact path and lifecycle compatibility locks without changing unrelated behavior.
- resume tests: verify artifact path and lifecycle compatibility locks without changing unrelated behavior.
- corruption tests: verify artifact path and lifecycle compatibility locks without changing unrelated behavior.
- manifest round-trip tests: verify artifact path and lifecycle compatibility locks without changing unrelated behavior.
- historical fixture tests: verify artifact path and lifecycle compatibility locks without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for artifact path and lifecycle compatibility locks.
...
- Update only terminology directly tied to artifact path and lifecycle compatibility locks.
...
- Prerequisites: TKT-001.
- Successors: TKT-027, TKT-025, TKT-029, TKT-033.
- Sibling tickets: TKT-001, TKT-002, TKT-003, TKT-005, TKT-006.
