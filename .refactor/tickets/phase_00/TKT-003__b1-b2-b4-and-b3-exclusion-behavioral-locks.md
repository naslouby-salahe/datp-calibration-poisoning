# TKT-003 — B1 B2 B4 and B3 Exclusion Behavioral Locks
...
- Blocks: TKT-048, TKT-050, TKT-049, TKT-051
...
Lock policy semantics before policy dispatch changes. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-001. Its primary ownership boundary is narrower than the parent epic: b1 b2 b4 and b3 exclusion behavioral locks.

## Problem Statement

Policy semantics are distinct from seed and artifact behavior and require independent tests and rollback.
...
- Source location: src/datp/core/poison_enums.py defines ThresholdPolicy with B1_GLOBAL, B2_PERSONALIZED, B4_CLUSTER only; src/datp/attacks/threshold_recompute.py owns B1/B2 pair computation; src/datp/attacks/b4_recompute.py owns B4 pair/decomposition; src/datp/attacks/guardrails.py has assert_policy_not_b3.
...
- Execution-time verification required: inspect the module responsible for b1 b2 b4 and b3 exclusion behavioral locks; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Policy flattening or B3 leakage changes the scientific interpretation of the whole matrix. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for b1 b2 b4 and b3 exclusion behavioral locks. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for b1 b2 b4 and b3 exclusion behavioral locks if the decision gate authorizes changes.
...
- Production changes unrelated to b1 b2 b4 and b3 exclusion behavioral locks.
...
- Intended canonical owner: the narrow package or module responsible for b1 b2 b4 and b3 exclusion behavioral locks, selected only after verification.
...
- unit tests: verify b1 b2 b4 and b3 exclusion behavioral locks without changing unrelated behavior.
- architecture/import tests: verify b1 b2 b4 and b3 exclusion behavioral locks without changing unrelated behavior.
- B4 determinism tests: verify b1 b2 b4 and b3 exclusion behavioral locks without changing unrelated behavior.
- policy semantic tests: verify b1 b2 b4 and b3 exclusion behavioral locks without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for b1 b2 b4 and b3 exclusion behavioral locks.
...
- Update only terminology directly tied to b1 b2 b4 and b3 exclusion behavioral locks.
...
- Successors: TKT-048, TKT-050, TKT-049, TKT-051.
- Sibling tickets: TKT-001, TKT-002, TKT-004, TKT-005, TKT-006.
