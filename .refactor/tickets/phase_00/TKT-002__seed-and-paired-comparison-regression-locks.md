# TKT-002 — Seed and Paired Comparison Regression Locks
...
Lock deterministic seed derivation and clean-versus-poisoned pairing behavior independently from policy and artifact tests. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-001. Its primary ownership boundary is narrower than the parent epic: seed and paired comparison regression locks.

## Problem Statement

Seed derivation and pair identity are independently reversible from artifact and policy locks, and a failure here invalidates every CP2 comparison.
...
- Source location: src/datp/core/seed_sequence.py uses SeedSequence entropy (training_seed, poisoning_seed, client_idx, scope_idx); src/datp/attacks/cell_runner.py calls make_seed_rng with collection.client_index(victim_id); src/datp/attacks/run_manifest.py persists SeedRecordModel entropy.
...
- Execution-time verification required: inspect the module responsible for seed and paired comparison regression locks; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Seed drift can make poisoned and clean runs incomparable even when all metrics look valid. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for seed and paired comparison regression locks. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for seed and paired comparison regression locks if the decision gate authorizes changes.
...
- Production changes unrelated to seed and paired comparison regression locks.
...
- Intended canonical owner: the narrow package or module responsible for seed and paired comparison regression locks, selected only after verification.
...
- unit tests: verify seed and paired comparison regression locks without changing unrelated behavior.
- architecture/import tests: verify seed and paired comparison regression locks without changing unrelated behavior.
- manifest round-trip tests: verify seed and paired comparison regression locks without changing unrelated behavior.
- historical fixture tests: verify seed and paired comparison regression locks without changing unrelated behavior.
- seed and pairing tests: verify seed and paired comparison regression locks without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for seed and paired comparison regression locks.
...
- Update only terminology directly tied to seed and paired comparison regression locks.
...
- Successors: TKT-008, TKT-018.
- Sibling tickets: TKT-001, TKT-003, TKT-004, TKT-005, TKT-006.
