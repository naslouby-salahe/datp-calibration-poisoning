# TKT-005 — Manifest Schema and Resume Compatibility Locks
...
- Blocks: TKT-031, TKT-034, TKT-037, TKT-065
...
Lock manifest round trips, provenance, mu_flag persistence, and resume eligibility. It can be implemented, tested, audited, rolled back, deferred, or rejected without completing all sibling work under TKT-001. Its primary ownership boundary is narrower than the parent epic: manifest schema and resume compatibility locks.

## Problem Statement

Manifests are a distinct compatibility boundary from path layout and metric formulas.
...
- Source location: src/datp/attacks/run_manifest.py defines RunManifest, ProvenanceRecord, SeedRecordModel; src/datp/attacks/bounded_sweep_manifest.py defines BoundedSweepManifest and BoundedSweepResultRow; src/datp/attacks/run_logger.py requires mu_flag_threshold before emit_manifest; tests/unit/attacks/test_run_logger.py and test_bounded_sweep_manifest.py cover parts of this behavior.
...
- Execution-time verification required: inspect the module responsible for manifest schema and resume compatibility locks; identify the exact constructor, call site, serializer, validator, or CLI route; confirm whether the concern exists.

## Failure Mechanism

Schema drift or weak resume checks can turn invalid artifacts into accepted scientific evidence. This mechanism is limited to the evidence above. Do not infer broader cleanup or unrelated package rewrites from this ticket.

## Target Outcome

The repository has one explicit, documented, and tested owner for manifest schema and resume compatibility locks. Existing scientific behavior, artifact paths, serialized values, and public interfaces are preserved unless this ticket explicitly authorizes an additive compatibility-safe migration.
...
- Make only the minimum source changes required for manifest schema and resume compatibility locks if the decision gate authorizes changes.
...
- Production changes unrelated to manifest schema and resume compatibility locks.
...
- Intended canonical owner: the narrow package or module responsible for manifest schema and resume compatibility locks, selected only after verification.
...
2. Action: classify the current owner and any duplicate owner. Target module or boundary: manifest schema and resume compatibility locks. Reason: distinguish intentional boundary projection from accidental duplication. Compatibility consideration: do not merge generic and CP2 concepts by name similarity. Validation expectation: classification matches `TICKET_OWNERSHIP_AND_DUPLICATION_RULES.md`.
...
- unit tests: verify manifest schema and resume compatibility locks without changing unrelated behavior.
- architecture/import tests: verify manifest schema and resume compatibility locks without changing unrelated behavior.
- lifecycle tests: verify manifest schema and resume compatibility locks without changing unrelated behavior.
- resume tests: verify manifest schema and resume compatibility locks without changing unrelated behavior.
- corruption tests: verify manifest schema and resume compatibility locks without changing unrelated behavior.
- manifest round-trip tests: verify manifest schema and resume compatibility locks without changing unrelated behavior.
- historical fixture tests: verify manifest schema and resume compatibility locks without changing unrelated behavior.
- seed and pairing tests: verify manifest schema and resume compatibility locks without changing unrelated behavior.
...
- Exactly one canonical owner or one documented intentional separation exists for manifest schema and resume compatibility locks.
...
- Update only terminology directly tied to manifest schema and resume compatibility locks.
...
- Successors: TKT-031, TKT-034, TKT-037, TKT-065.
- Sibling tickets: TKT-001, TKT-002, TKT-003, TKT-004, TKT-006.
