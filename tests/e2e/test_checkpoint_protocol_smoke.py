"""End-to-end smoke tests for the checkpoint protocol CLI command."""
...
def test_checkpoint_protocol_smoke_cli_uses_temp_root(tmp_path: Path) -> None:
    """Verify that running checkpoint-protocol smoke with a temp root successfully runs rounds and writes scores/results."""
    runner = CliRunner()
    ...
    assert (tmp_path / "scores" / "nbaiot_main" / "seed_0" / "round_25").is_dir()
    assert (
        tmp_path
        / "results"
        / "nbaiot_main"
...
def test_checkpoint_protocol_smoke_cli_rejects_outputs_child() -> None:
    """Verify that checkpoint-protocol smoke rejects specifying a root path inside 'outputs'."""
    runner = CliRunner()
