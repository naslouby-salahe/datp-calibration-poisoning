"""Tests verifying training convergence monitors, status detection, and summary JSON payload builds."""
...
from datp.validation.results import ConvergencePayload, convergence_payload
...
def _write_summary(
    ...
    *,
    ...
    """Helper to write a mock convergence summary JSON file to the target checkpoint directory."""
    summary = {
...
def _write_curve(ckpt_dir: Path) -> Path:
    """Helper to write a mock convergence curve CSV file to the target checkpoint directory."""
...
class TestConvergencePayloadMissingCheckpoint:
    """Tests verifying convergence payload behavior when checkpoint files are missing."""

    def test_no_checkpoint_no_summary(self, tmp_path: Path) -> None:
        """Verify ConvergenceStatus.MISSING_CHECKPOINT is returned if no checkpoint or summary exists."""
    ...
    def test_checkpoint_exists_no_summary(self, tmp_path: Path) -> None:
        """Verify ConvergenceStatus.BLOCKED_PENDING_RUN is returned if checkpoint exists but summary does not."""
    ...
    def test_checkpoint_exists_no_summary_no_curve(self, tmp_path: Path) -> None:
        """Verify curve path is None if summary file is missing."""
        ckpt_dir = tmp_path / "checkpoints/a/seed_0"
        ckpt_dir.mkdir(parents=True)
        checkpoint = ckpt_dir / ArtifactFile.MODEL_CHECKPOINT
        checkpoint.write_bytes(b"model")
...
class TestConvergencePayloadWithSummary:
    ...
    def test_curve_present_but_summary_absent_is_blocked(self, tmp_path: Path) -> None:
        """Verify status is BLOCKED_PENDING_RUN and curve path is ignored if summary is absent."""
        ckpt_dir = tmp_path / "checkpoints/a/seed_0"
        ckpt_dir.mkdir(parents=True)
        checkpoint = ckpt_dir / ArtifactFile.MODEL_CHECKPOINT
        checkpoint.write_bytes(b"model")
