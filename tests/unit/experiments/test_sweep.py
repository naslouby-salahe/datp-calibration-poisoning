"""Unit tests for experiment sweep planning and execution."""
...
from datp.experiments.sweep import validate_sweep
...
def _checkpoint_protocol() -> CheckpointProtocolConfig:
...
class TestCheckpointProtocolCompletion:
    """Checkpoint protocol completion tracking."""

    def test_non_round_metrics_not_counted_when_checkpoint_protocol_enabled(
        ...
        cfg = BASE_CONFIG.model_copy(
            update={"checkpoint_protocol": _checkpoint_protocol()}
        )
        monkeypatch.setattr(sweep_module, "BASE_CONFIG", cfg)
        run = PolicyRunId(
            cell=TrainingCellId(stage=_STAGE, seed=0),
            policy=ThresholdPolicy.GLOBAL_THRESHOLD,
        )
        non_round_result_dir = (
            ArtifactLayout(base_dir=tmp_path, stage=_STAGE).policy_run(run).result_dir
        )
        non_round_result_dir.mkdir(parents=True, exist_ok=True)
        (non_round_result_dir / "metrics.json").write_text(
            valid_metrics_json("global_threshold", "nbaiot_main", 0)
        )

        assert _cell_is_done(run, tmp_path) is False

    def test_shared_baseline_done_requires_all_checkpoint_rounds(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ):
        from datp.artifacts.layout import ArtifactLayout
        from datp.config.compose import BASE_CONFIG
        ...
        cfg = BASE_CONFIG.model_copy(update={"checkpoint_protocol": protocol})
        monkeypatch.setattr(sweep_module, "BASE_CONFIG", cfg)
        run = PolicyRunId(
            cell=TrainingCellId(stage=_STAGE, seed=0),
            policy=ThresholdPolicy.GLOBAL_THRESHOLD,
...
class TestRunSweep:
    ...
    def test_skips_completed_runs(self, tmp_path: Path):
        ...
        with (
            patch("datp.experiments.sweep.init_tracking"),
            patch(
