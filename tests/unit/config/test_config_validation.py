"""Unit tests for DatpConfig validation rules and error messages."""
...
class TestInputDimMismatch:
    """Input dimension mismatch between model and dataset."""
    ...
    def test_mismatched_dims_fail(self) -> None:
        with pytest.raises(ValidationError, match="model.input_dim"):
            ...
            bad["model"]["input_dim"] = 99
            type(BASE_CONFIG).model_validate(bad)


class TestUnknownKeys:
    """Unknown keys in config raise validation errors."""
    ...
    def test_missing_required_key_fails(self) -> None:
        with pytest.raises(ValidationError):
            ModelConfig.model_validate({"input_dim": None})
...
class TestOverrideNotStripped:
    """Override fields are preserved after validation."""
...
class TestCrossSectionInvariants:
    """Cross-section invariants across config fields."""
...
class TestMachineProfile:
    """Machine profile detection and validation."""
