"""Unit tests for the FedAvg federated training protocol."""
...
class TestRunFlTrainingValidation:
    """run_fl_training input validation."""
...
class TestRunFlTrainingRouting:
    """run_fl_training routes to the correct protocol implementation."""

    def _capture_sim_calls(self, monkeypatch: pytest.MonkeyPatch) -> list[dict]:
        from datp.federated.simulation import FlSimulationRequest

        captured: list[dict] = []
        import datp.federated.protocols.fedavg as fedavg_mod

        def fake_sim(req: FlSimulationRequest) -> object:
            captured.append(
                {
    ...
    def _make_cfg(self, stage: ExperimentStage = _STAGE) -> DatpConfig:
