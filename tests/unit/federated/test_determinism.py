"""Tests verifying end-to-end simulation reproducibility and seed sensitivity."""
...
def _run_experiment(seed: int | None, run_dir: Path) -> Path:
    """Helper to run a Flower smoke simulation and output the metrics file."""
    metrics = {
        "losses_distributed": [
...
def test_determinism_same_seed_identical_metrics(tmp_path: Path) -> None:
    """Verify that running the simulation twice with the same seed yields identical metrics."""
...
def test_determinism_guard_no_seeds_differ(tmp_path: Path) -> None:
    """Verify that running the simulation with different seeds yields different metrics."""
    path_a = _run_experiment(seed=42, run_dir=tmp_path / "run_a")
