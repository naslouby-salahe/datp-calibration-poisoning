...
from datp.attacks.metrics import compute_metrics
from datp.attacks.types import MetricInput
...
def _make_setup(fraction: float = 0.40):
    ...
    global_result = compute_metrics(
        MetricInput(collection=col, pair=global_pair, mu_flag_threshold=None)
    )
    local_result = compute_metrics(
        MetricInput(collection=col, pair=local_pair, mu_flag_threshold=None)
    )
