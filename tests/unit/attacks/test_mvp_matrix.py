"""Unit tests for the locked bounded-MVP cell-matrix enumerator (CP2-T045)."""

from __future__ import annotations

import pytest

from datp.attacks.mvp_matrix import enumerate_mvp_matrix
...
def test_matrix_size_is_exactly_1620():
    cells = enumerate_mvp_matrix(_VICTIMS_BY_SEED)
...
def test_matrix_sources_are_exactly_mvp_three():
    cells = enumerate_mvp_matrix(_VICTIMS_BY_SEED)
...
def test_matrix_missing_seed_raises_keyerror():
    ...
    with pytest.raises(KeyError):
        enumerate_mvp_matrix(incomplete)
