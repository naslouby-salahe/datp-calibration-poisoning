...
class TestFitScaler:
    def test_fitted_on_train_only(self) -> None:
        ...
        np.testing.assert_allclose(scaler.mean_, train_df.to_numpy().mean(axis=0), atol=1e-10)  # type: ignore[arg-type]
        np.testing.assert_allclose(scaler.scale_, train_df.to_numpy().std(axis=0, ddof=0), atol=1e-10)  # type: ignore[arg-type]
