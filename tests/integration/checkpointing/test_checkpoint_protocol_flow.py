"""Integration test for the full checkpoint → scoring → evaluation → summary pipeline."""
...
def _client_data() -> dict[str, ClientData]:
    """Build two minimal tensor-based clients for checkpoint-protocol testing."""
    return {
        "c1": ClientData(
...
def test_checkpoint_scoring_evaluation_summary_and_status(tmp_path: Path) -> None:
    """Scoring at two checkpoint rounds, followed by evaluation and status reporting, completes without error."""
