...
from datp.attacks.sweep_run import (
    execute_bounded_sweep,
    write_bounded_sweep_manifest,
)
...
def test_execute_bounded_sweep_sweep_end_to_end(tmp_path) -> None:
    cfg = _make_cfg()
    ...
    manifest = execute_bounded_sweep(base_dir=tmp_path, config=_CONFIG)
...
@pytest.mark.integration
def test_write_bounded_sweep_manifest_writes_canonical_path(tmp_path) -> None:
    cfg = _make_cfg()
    for training_seed in _CONFIG.seeds.training:
        set_seeds(training_seed)
        client_data = _make_client_data(training_seed)
    ...
    out_path = write_bounded_sweep_manifest(tmp_path)

    assert out_path.name == "nbaiot_main_manifest.json"
    assert out_path.exists()


def test_execute_bounded_sweep_sweep_config_is_required() -> None:
    """config must be a required parameter with no default fallback."""
    import inspect

    sig = inspect.signature(execute_bounded_sweep)
    config_param = sig.parameters["config"]
    assert config_param.default is inspect.Parameter.empty, (
        "execute_bounded_sweep must not have a default config — "
