"""Integration tests for NBAIOT data preparation, split ratios, and gap logic."""
...
class TestSplitRatio:
    """Verify split ratios for train/cal/test match expected counts on real data."""
...
class TestGapContiguous:
    """Verify gap indices are contiguous with no overlaps across splits."""
...
@pytest.mark.integration
class TestNoLeak:
    """Verify no data leakage across train/cal/test splits."""
...
class TestCalibrationCounts:
    """Verify per-device calibration counts and structural properties."""
